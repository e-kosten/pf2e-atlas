...
#[test]
fn contract_v8_is_rejected_even_with_current_schema() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(crate::ARTIFACT_SCHEMA_VERSION, "5");
    assert_eq!(
    ...
    assert!(
        report
            .diagnostics
            .iter()
            .any(|diagnostic| diagnostic.expected.as_deref() == Some("pf2e-atlas-artifact/v9"))
    );
    fs::remove_file(path)?;
    Ok(())
}
