...
use atlas_artifact::test_support::{create_minimal_contract_schema, insert_minimal_contract_rows};
use atlas_embedding::EmbeddingModelId;
use rusqlite::Connection;

use super::{
    create_contract_database, create_contract_database_with_override,
    create_contract_database_without, insert_contract_metadata, insert_contract_metadata_entries,
    temp_db_path, valid_metadata_entries_for_embedding,
};
use crate::{ArtifactContractFamily, SqliteIndexReader, ValidationCode, ValidationStatus};
...
fn reports_valid_artifact_metadata() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        ...
        Some(atlas_artifact::metadata::ARTIFACT_CONTRACT_VERSION)
    );
    fs::remove_file(path)?;
    Ok(())
}
