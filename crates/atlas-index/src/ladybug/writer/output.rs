...
pub(crate) fn ladybug_write_error(error: impl std::fmt::Display) -> IndexWriteError {
    IndexWriteError::WriteFailed(format!("LadybugDB write failed: {error}"))
}
...
impl LadybugOutput {
    pub(crate) fn prepare(target_path: &Path) -> Result<Self, IndexWriteError> {
    ...
    pub(crate) fn commit(self) -> Result<(), IndexWriteError> {
...
fn temp_ladybug_path(target_path: &Path) -> Result<PathBuf, IndexWriteError> {
    let parent = target_path.parent().unwrap_or_else(|| Path::new(""));
    let file_name = target_path
        .file_name()
        .ok_or_else(|| {
            IndexWriteError::WriteFailed("LadybugDB output path has no file name".to_string())
        })?
        .to_string_lossy();
...
fn remove_path_if_exists(path: &Path) -> Result<(), IndexWriteError> {
    match fs::symlink_metadata(path) {
        Ok(metadata) if metadata.is_dir() => fs::remove_dir_all(path)
            .map_err(|error| IndexWriteError::WriteFailed(error.to_string())),
        Ok(_) => {
            fs::remove_file(path).map_err(|error| IndexWriteError::WriteFailed(error.to_string()))
        }
...
fn remove_ladybug_files(path: &Path) -> Result<(), IndexWriteError> {
...
fn move_ladybug_files(source_path: &Path, target_path: &Path) -> Result<(), IndexWriteError> {
