...
pub(crate) fn run_graph_links(options: GraphLinksOptions) -> Result<ExitCode, String> {
    ...
    let result = match service.graph_context(
        GraphContextRequest::new(key.clone())
            .with_outgoing_limit(options.outgoing)
            .with_backlink_limit(options.backlinks),
    ) {
    ...
    let data = graph_links_data(&result, options.detail).map_err(|error| error.to_string())?;
    if options.json {
