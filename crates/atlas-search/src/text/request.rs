...
impl TextSearchTuning {
    pub fn default_for_page(offset: u32, limit: u32) -> Result<Self, SearchError> {
        Self::resolve_for_page(None, offset, limit)
    }
    ...
    pub fn resolve_for_page(
        ...
        let required_window = offset.saturating_add(limit);
        ...
        validate_text_search_tuning(&tuning, offset, limit)?;
        Ok(tuning)
    }
}
...
pub(super) fn validate_search_text_request(
    ...
    TextSearchTuning::resolve_for_page(request.tuning, request.offset, request.limit)
}
...
fn validate_text_search_tuning(
    ...
    let ranked_window = offset.saturating_add(limit);
        ...
        || tuning.vector_top_k > MAX_RANKED_CANDIDATE_WINDOW
    {
        return Err(SearchError::invalid_search_options(format!(
            "ranked search candidate windows must be at most {MAX_RANKED_CANDIDATE_WINDOW}; got offset {offset}, limit {limit}, fts_top_k {}, vector_top_k {}",
            tuning.fts_top_k, tuning.vector_top_k
        )));
    }
    Ok(())
}
