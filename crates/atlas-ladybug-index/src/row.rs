...
use atlas_index::{DiscoveryError, GraphReferenceEdge, VectorSearchHit};
use atlas_record::{
...
use crate::{LadybugIndexError, invalid};

pub(crate) fn record_from_row(row: &[Value]) -> Result<PersistedRecord, LadybugIndexError> {
...
pub(crate) fn alias_from_row(row: &[Value]) -> Result<RecordAlias, LadybugIndexError> {
    Ok(RecordAlias {
...
pub(crate) fn reference_edge_from_row(row: &[Value]) -> Result<ReferenceEdge, LadybugIndexError> {
    Ok(ReferenceEdge {
...
pub(crate) fn remaster_link_from_row(row: &[Value]) -> Result<RemasterLink, LadybugIndexError> {
    Ok(RemasterLink {
...
pub(crate) fn vector_hit_from_row(row: &[Value]) -> Result<VectorSearchHit, LadybugIndexError> {
    Ok(VectorSearchHit {
...
pub(crate) fn record_key_at(row: &[Value], index: usize) -> Result<RecordKey, LadybugIndexError> {
    RecordKey::parse(&string_at(row, index)?).map_err(invalid)
}

pub(crate) fn string_at(row: &[Value], index: usize) -> Result<String, LadybugIndexError> {
    match row.get(index) {
...
pub(crate) fn optional_string_at(
    row: &[Value],
    index: usize,
) -> Result<Option<String>, LadybugIndexError> {
    match row.get(index) {
...
pub(crate) fn int_at(row: &[Value], index: usize) -> Result<i64, LadybugIndexError> {
    match row.get(index) {
...
pub(crate) fn u64_at(row: &[Value], index: usize) -> Result<u64, LadybugIndexError> {
...
pub(crate) fn optional_i64_at(
    row: &[Value],
    index: usize,
) -> Result<Option<i64>, LadybugIndexError> {
    if index == usize::MAX {
        return Ok(None);
    }
    match row.get(index) {
...
pub(crate) fn optional_bool_at(
    row: &[Value],
    index: usize,
) -> Result<Option<bool>, LadybugIndexError> {
    match row.get(index) {
        Some(Value::Null(_)) => Ok(None),
...
pub(crate) fn bool_at(row: &[Value], index: usize) -> Result<bool, LadybugIndexError> {
    match row.get(index) {
...
pub(crate) fn float_at(row: &[Value], index: usize) -> Result<f64, LadybugIndexError> {
    match row.get(index) {
...
pub(crate) fn discovery_error(error: LadybugIndexError) -> DiscoveryError {
...
pub(crate) fn optional_float_at(
    row: &[Value],
    index: usize,
) -> Result<Option<f64>, LadybugIndexError> {
    match row.get(index) {
        Some(Value::Null(_)) => Ok(None),
...
pub(crate) fn optional_actor_data(
    row: &[Value],
) -> Result<Option<ActorSideData>, LadybugIndexError> {
...
pub(crate) fn optional_item_data(row: &[Value]) -> Result<Option<ItemSideData>, LadybugIndexError> {
...
pub(crate) fn optional_spell_data(
    row: &[Value],
) -> Result<Option<SpellSideData>, LadybugIndexError> {
