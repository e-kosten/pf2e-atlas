...
use atlas_index::{
    ...
    FtsColumnWeights, FtsQuery, FtsSearchHit, GraphReferenceEdge, ReferenceEdgeDirection,
    VectorSearchHit,
};
use atlas_record::{
...
pub(crate) fn unsupported<T>(feature: &str) -> Result<T, LadybugIndexError> {
    Err(LadybugIndexError::Unsupported(format!(
        "{feature} is not implemented in the Ladybug read spike yet"
    )))
}

pub(crate) fn invalid(error: impl std::fmt::Display) -> LadybugIndexError {
