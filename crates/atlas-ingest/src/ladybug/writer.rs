...
use atlas_record::{
    ContentBlock, ContentDocument, ContentReference, ContentReferenceLocator, ContentSectionNode,
    ContentSectionOrigin, ContentSourceKind, ContentVisibility, MetricRow, NormalizedRecord,
...
fn create_schema(connection: &Connection<'_>, dimensions: usize) -> Result<(), IngestError> {
    for statement in [
...
fn evidence_units(records: &[&NormalizedRecord]) -> Vec<EvidenceUnit> {
...
fn write_parquet(
    ...
    let batch = RecordBatch::try_new(schema.clone(), arrays)
        .map_err(|error| IngestError::ArtifactWriteFailed(error.to_string()))?;
    let file =
        File::create(path).map_err(|error| IngestError::ArtifactWriteFailed(error.to_string()))?;
...
fn arrow_field(name: &str, data_type: DataType, nullable: bool) -> Field {
    Field::new(name, data_type, nullable)
}

fn arrow_strings(values: impl IntoIterator<Item = String>) -> ArrayRef {
    Arc::new(StringArray::from_iter_values(values))
}

fn arrow_optional_strings(values: impl IntoIterator<Item = Option<String>>) -> ArrayRef {
    Arc::new(StringArray::from_iter(values))
}

fn arrow_ints(values: impl IntoIterator<Item = i64>) -> ArrayRef {
    Arc::new(Int64Array::from_iter_values(values))
}

fn arrow_optional_ints(values: impl IntoIterator<Item = Option<i64>>) -> ArrayRef {
    Arc::new(Int64Array::from_iter(values))
}

fn arrow_bools(values: impl IntoIterator<Item = bool>) -> ArrayRef {
    Arc::new(BooleanArray::from_iter(values.into_iter().map(Some)))
}

fn arrow_optional_bools(values: impl IntoIterator<Item = Option<bool>>) -> ArrayRef {
    Arc::new(BooleanArray::from_iter(values))
}

fn arrow_optional_floats(values: impl IntoIterator<Item = Option<f64>>) -> ArrayRef {
...
fn arrow_fixed_f32_lists<'a>(
    rows: impl IntoIterator<Item = &'a [f32]>,
    dimensions: usize,
) -> Result<ArrayRef, IngestError> {
...
fn recreate_dir(path: &Path) -> Result<(), IngestError> {
    if path.exists() {
        fs::remove_dir_all(path)
            .map_err(|error| IngestError::ArtifactWriteFailed(error.to_string()))?;
    }
    fs::create_dir_all(path).map_err(|error| IngestError::ArtifactWriteFailed(error.to_string()))
}

fn create_search_indexes(
    connection: &Connection<'_>,
    create_vector_index: bool,
) -> Result<(), IngestError> {
    let total_steps = if create_vector_index { 5 } else { 3 };
    let mut step = 1;
    ...
    ladybug_search_index_progress(
        step,
        total_steps,
        "Creating LadybugDB SearchDocument FTS index",
    );
    connection
        .query(
...
fn load_extension(connection: &Connection<'_>, extension: &str) -> Result<(), IngestError> {
...
fn ladybug_progress(phase: &'static str, message: &'static str) {
    info!(target: "atlas_progress", phase, "{message}");
}

fn ladybug_progress_message(phase: &'static str, message: impl std::fmt::Display) {
    info!(target: "atlas_progress", phase, "{}", message);
}

fn ladybug_search_index_progress(step: usize, total_steps: usize, message: &'static str) {
    ladybug_progress_message(
...
fn ladybug_write_error(error: impl std::fmt::Display) -> IngestError {
    IngestError::ArtifactWriteFailed(format!("LadybugDB write failed: {error}"))
}

struct LadybugOutput {
    target_path: PathBuf,
    temp_path: PathBuf,
