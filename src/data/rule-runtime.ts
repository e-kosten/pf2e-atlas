...
function collectDirectionGraph(
  direction: RuleReferenceEdge["direction"],
  recordKeys: string[],
  { coreOnly = false, maxPerPrimary = 4 }: { coreOnly?: boolean; maxPerPrimary?: number } = {},
  ...
  for (const primaryKey of recordKeys) {
    ...
    bucket.sort((left, right) => {
      const leftTypeRank = left.fromPackName === "classfeatures" ? 2 : backlinkTypeRank(left.fromRecordType);
      const rightTypeRank = right.fromPackName === "classfeatures" ? 2 : backlinkTypeRank(right.fromRecordType);
...
export function collectRuleQuestionContext(
  ...
  const derivedRules =
    explicitRules.length > 0 ? explicitRules : input.question ? extractQuestionRuleNames(input.question) : [];
  const primary = deps.lookupMany(
    derivedRules.map((name) => ({ name })),
