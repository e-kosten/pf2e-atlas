...
export async function loadConfig(argv = process.argv.slice(2), env = process.env): Promise<AppConfig> {
  ...
  const configuredRankingConfigWatch = parseOptionalBoolean(
    args["ranking-config-watch"] ?? env.PF2E_RANKING_CONFIG_WATCH,
  );
  ...
  const configuredEmbeddingCachePath =
    args["embedding-cache-path"] ?? env.PF2E_EMBEDDING_CACHE_PATH ?? path.join(path.dirname(indexPath), "hf-models");
    ...
    if (await pathIsReadable(candidate)) {
      return {
        ...
        ranking: {
          ...
          watch: configuredRankingConfigWatch ?? false,
        },
      };
    }
  }
