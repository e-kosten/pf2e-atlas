...
| `pf2e_get_rule_graph` | Rule graph records and edges for known canonical record keys | `atlas graph links <record-key>` over one known key with outgoing/backlink limits and edge evidence. V1 is one-hop, key-based, and retrieval-only. |
