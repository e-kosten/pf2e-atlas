...
| `NormalizedRecord` | `RecordSummary`, later `RecordDetail`, side-data structs | `atlas-domain` | rust redesign | `RecordSummary` owns lightweight list/preview facts: identity, name, category/subcategory, optional level/rarity/action cost, traits, publication/source summary, text status, and optional summary text. Avoid one overgrown public struct when command outputs need smaller typed envelopes. |
