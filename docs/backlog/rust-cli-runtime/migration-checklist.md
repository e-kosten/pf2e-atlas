...
- [x] Decide that persisted config lookup is not part of Phase 8. `--path-mode global|repo` plus direct command-local path flags are the current path contract; any future persisted config should feed runtime path overrides below direct CLI flags.
