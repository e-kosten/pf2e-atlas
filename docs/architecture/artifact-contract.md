...
The target stores canonical entities, contextual occurrences, owned content, exact reference occurrences, and derived query projections with stable identities. Product-addressable activities, spellcasting entries, resources, content, and occurrences are relational. Record metrics, activities, and spellcasting entries retain the canonical vector order through non-null parent-scoped ordinals. Activity rows use `(record_key, ordinal)` as their internal storage identity because canonical parent-local `activity_id` values may repeat; the unchanged non-unique `activity_id` remains typed payload data and is not exposed as a storage locator. Spellcasting entries retain their parent/entry-ID identity. Both payload families use deterministic Atlas-owned typed JSON. Independently queryable/filterable/joinable facts require authoritative relational projections. Complete canonical hydration belongs only to `atlas-index::read`.
...
Builds stage and sync the complete SQLite file and manifest before publication. After writer connections close and WAL/SHM companions are absent, the writer seals the staged file read-only and creates one opaque live validation receipt. Deep validation and the writer SHA-256 use the same retained open-file identity; device/inode on Unix and volume/file-index on Windows stay behind the index-owned identity abstraction. The receipt also binds the staged path, intended publication target, byte count, modification token, and successful validation result. It cannot be serialized or reconstructed from evidence. Ingest may read only its digest for the unchanged manifest and must pass the live receipt back to the index publisher.

Publishers for the same adjacent-manifest target serialize through an exclusive OS lock whose kernel ownership is released after process failure; the coordination file itself remains stable so deleting and recreating it cannot split lock domains. Exclusive acquisition has a five-second deadline and returns actionable retry guidance instead of blocking indefinitely. Under that lock, publication rejects receipt identity or manifest drift, recovers or cleans fixed transaction backups from an interrupted attempt, copies bytes from the retained verified handle into an absent digest-named generation, syncs and independently hashes that copy, and proves the generation has a distinct file identity. Hard links, reflinks, and other shared-mutation aliases are not generation materialization. Publication then snapshots a prior matching pair, installs both files, verifies the renamed visible file through the still-live receipt identity plus manifest digest without rereading unchanged bytes, and removes recovery state. Recovery and prior-pair verification never trust serialized receipt evidence and retain independent full hashing. Failure before commit restores both prior files or removes both first-publication files. Readers share the lock only during generation acquisition, never for the long-lived reader or service lifetime. Completed publication removes obsolete generation snapshots when no live operating-system handle prevents deletion; a final old-generation reader also removes its obsolete snapshot on drop. Orphan generation and temporary files from an interrupted attempt are cleaned by the next successful publication, and stale staging files are never selected as visible state.
...
| Ordered record mechanics | `record_metrics`, `record_activities`, `record_spellcasting_entries`, `metric_key_catalog`, `metric_value_catalog` | Open-ended actor/item metrics retain semantic-key uniqueness and canonical parent-scoped ordinal order. Activity rows use parent/ordinal storage identity while preserving non-unique canonical `activity_id` payload values unchanged; spellcasting entries retain parent/entry-ID identity. Both child tables cascade with record ownership and preserve canonical typed payloads for already-modeled nested fields. Generated metric discovery catalogs cover unfiltered and single-record-kind scopes by metric domain and namespace. |
...
`records_fts` structured-term columns are deterministic projections from normalized typed record fields, not parsed raw JSON. For canonical creature bodies, `mechanic_terms` and `metric_terms` consume `project_creature_mechanics` directly, including canonical values for defenses, awareness/skills, movement, resources, strikes, actions, spellcasting, spells, rolls/DCs, damage, action costs, frequencies, uses, and spell slots; canonical actor-side categories such as senses and IWR come from `project_creature_facts`, and conflicting legacy `AtlasRecord.mechanics` values do not enter those columns. `taxonomy_terms` contains categorical identity fields such as record kind, Foundry record type, taxonomy families, item category/group/base item, spell kinds, and variant labels. `constraint_terms` contains user-facing preconditions and use gates such as prerequisites, activation/action-cost phrases, and hands requirements. Non-canonical record families continue to project their normalized structured mechanics through the established record projection. `source_terms` contains publication title/family and pack label, not pack name.
...
- Actor, item, and spell side tables hold stable non-metric filter and presentation data. Metrics remain in semantic-key-unique `record_metrics` with explicit canonical-vector ordinals. Already-modeled record activities and spellcasting entries live in their parent-keyed ordered child tables; direct Foundry item projections keep `system_*` names.
