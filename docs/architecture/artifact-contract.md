...
Runtime artifact validation requires the relative `adjacent_manifest_path` metadata, a manifest-bound matching SQLite digest, and SQLite coherence. Reader acquisition takes a shared lock on the persistent pair-coordination file, opens the SQLite file once, hashes that open file, and opens every Diesel and validation/hydration SQLite connection while the lock still protects that verified generation. Those connections are retained rather than reopening the visible pathname. Unix readers bind them through the retained file descriptor and may release the acquisition lock; targets without a descriptor-backed SQLite path retain the shared lock until all reader connections and the verified file handle are dropped. A publisher therefore cannot replace the visible pair between verification and connection open or while a path-bound reader is active, and no reader can combine connections from different generations. Setup owns freshness policy by reading the adjacent manifest, verifying that its source and record counts match artifact metadata, comparing the current source position, and deciding whether full source analysis is needed before planning a rebuild.

Builds stage and sync the complete SQLite file and manifest before publication. Publishers for the same adjacent-manifest target serialize through an exclusive OS lock whose kernel ownership is released after process failure; the coordination file itself remains stable so deleting and recreating it cannot split lock domains. Under that lock, publication verifies the staged digest, recovers or cleans fixed transaction backups from an interrupted attempt, snapshots a prior matching pair, installs both files, re-verifies the final digest, and removes recovery state. Failure before commit restores both prior files or removes both first-publication files. Readers share the same lock during pair acquisition; path-bound readers retain it for their connection lifetime, so a publisher waits instead of attempting a replacement that the operating system could reject. Readers therefore either retain a previously verified generation or wait for the complete new pair; stale staging files are never selected as visible state.
