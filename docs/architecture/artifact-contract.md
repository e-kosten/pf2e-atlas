...
Runtime artifact validation requires the relative `adjacent_manifest_path` metadata, a manifest-bound matching SQLite digest, and SQLite coherence. Reader acquisition takes a shared lock on the persistent pair-coordination file, opens the SQLite file once, hashes that open file, and opens SQLite through that same immutable file generation. Replacing the visible SQLite path after verification therefore cannot change the database generation returned by that reader. Setup owns freshness policy by reading the adjacent manifest, verifying that its source and record counts match artifact metadata, comparing the current source position, and deciding whether full source analysis is needed before planning a rebuild.

Builds stage and sync the complete SQLite file and manifest before publication. Publishers for the same adjacent-manifest target serialize through an exclusive OS lock whose kernel ownership is released after process failure; the coordination file itself remains stable so deleting and recreating it cannot split lock domains. Under that lock, publication verifies the staged digest, recovers or cleans fixed transaction backups from an interrupted attempt, snapshots a prior matching pair, installs both files, re-verifies the final digest, and removes recovery state. Failure before commit restores both prior files or removes both first-publication files. Readers share the same lock during pair acquisition, so they either open a previously verified generation or wait for the complete new pair; stale staging files are never selected as visible state.
