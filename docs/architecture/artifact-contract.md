...
Physical SQLite table and column ownership lives in `atlas-index`. Diesel migrations and Diesel schema declarations are the intended source for ordinary relational tables, writer DTOs, and stable reader hydration. `atlas-index` also owns artifact metadata keys, expected artifact contract values, narrow validation requirements for external artifact files, runtime validation semantics, and SQLite `f32` vector blob encoding. Raw SQL remains an index-owned implementation detail for SQLite facilities that do not fit Diesel's typed query model: FTS5 virtual tables, sqlite-vec virtual tables, dynamic filter/discovery relations, and SQLite validation pragmas. Raw-SQL helpers may keep stable table and column name constants, but they must not recreate a parallel schema descriptor model for Diesel-owned tables. Artifact validation report DTOs live with the validator in `atlas-index`. `atlas-domain` owns semantic vocabulary and shared request/output contracts; it must not own SQLite DDL, table inventories, artifact metadata inventories, storage DTOs, or validation-context DTOs. `atlas-discovery` owns shared filter discovery field policy, including field ids, value policies, operators, CLI flag mappings, applicability, and the source SQL projections used to generate discovery catalogs.
