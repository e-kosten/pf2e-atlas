...
| Records | `records` | Canonical normalized record identity, derived `record_family`, source path/provenance, selected direct `system_*` projections, normalized price/time facts, variant facts, default retrieval visibility, `description_json`, `blurb_json`, and raw JSON for parity/debugging. |
