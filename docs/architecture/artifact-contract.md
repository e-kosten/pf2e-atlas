...
The v3 contract writer publishes only after the completed temporary SQLite artifact passes cheap compatibility checks and produces its transfer digest. Deep validation remains explicit supplementary tooling. `atlas-index::read` decodes the canonical typed body and exposes canonical-body and combined hydrated-record reads; no other crate owns complete hydration. Old artifacts receive explicit rebuild guidance and are never upgraded in place.
...
| --- | --- |
| `artifact_contract_version` | `pf2e-atlas-artifact/v5` |
| `schema_version` | `3` |
...
| Ordered query metrics | `record_metrics`, `metric_key_catalog`, `metric_value_catalog` | Open-ended actor/item metrics retain semantic-key uniqueness and canonical parent-scoped ordinal order. Creature metrics are one-way query projections from `CreatureRecord`, not a hydrated presentation/runtime model. Creature activities and spellcasting live only in the canonical creature body plus its entity/occurrence tables. Generated metric discovery catalogs cover unfiltered and single-record-kind scopes by metric domain and namespace. |
...
`records_fts` structured-term columns are deterministic projections from normalized typed record fields, not parsed raw JSON. For canonical creature bodies, `mechanic_terms` and `metric_terms` consume `project_creature_mechanics` directly, including canonical values for defenses, awareness/skills, movement, resources, strikes, actions, spellcasting, spells, rolls/DCs, damage, action costs, frequencies, uses, and spell slots; categorical terms such as senses and IWR read the same canonical creature body directly. Generic `AtlasRecord.mechanics` does not enter creature FTS columns. `taxonomy_terms` contains categorical identity fields such as record kind, Foundry record type, taxonomy families, item category/group/base item, spell kinds, and variant labels. `constraint_terms` contains user-facing preconditions and use gates such as prerequisites, activation/action-cost phrases, and hands requirements. Non-creature record families continue to project their normalized structured mechanics through the established record projection. `source_terms` contains publication title/family and pack label, not pack name.

Document embedding preparation selects ordinary records using `atlas-record::ProductRetrievalPolicy`, the same typed decision persisted by the artifact writer. For canonical creature bodies the parent input begins with canonical mechanics and consumes unique owned rich-content documents across record, entity, and occurrence ownership and typed visibility classifications. Content marked `CopiedFromCanonicalTarget` is omitted so copied embedded spell/item prose cannot dominate the parent input or duplicate the canonical target; unique embedded authored content remains eligible. Existing structured chunk ordering, token budgeting, semantic-input hashes, and overflow-only child-unit materialization remain the embedding contract.
...
- Actor, item, and spell side tables hold stable non-metric filter and presentation data for their explicitly non-creature owners. Metrics remain in semantic-key-unique `record_metrics` with explicit canonical-vector ordinals. Creature activities and spellcasting entries live only in the canonical body and its entity/occurrence tables; direct Foundry item projections keep `system_*` names.
