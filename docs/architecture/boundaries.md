...
| Metadata field ownership         | `src/domain/metadata-field-catalog.ts`, `src/search/filters/metadata-execution.ts`, `src/data/metadata-row-projection.ts`, and `src/server/metadata-presentation.ts` by concern                                                                            | App, domain, server, TUI, and data modules cannot import search-owned metadata execution helpers                                                                                                | Use the domain catalog for public field meaning, data projection for row hydration, search execution helpers only inside search, and server presentation helpers for MCP summaries                                |
...
- `src/search/filters/` owns normalization, validation, and metadata execution helpers for execution filters; it does not own public metadata field semantics, row hydration, or MCP presentation projection
