...
- `src/data/` owns physical retrieval mechanics and `src/search/` owns storage-agnostic search execution mechanics.
...
| SQLite construction              | `src/data/schema.ts`, `src/app/storage-service.ts`, `src/refresh-index.ts`, and `src/tags/cli/**`                                                                                                                                                         | `arch/no-direct-database-sync-construction` bans `new DatabaseSync(...)` almost everywhere else                                                                                                 | Use `Pf2eDataService`, `Pf2eApplicationStorageService`, or another approved facade                                                                                                                                                       |
...
| Server-to-storage shortcuts      | No server registration files                                                                                                                                                                                                                              | `src/server/**/*` cannot import low-level data SQL, query, or schema modules                                                                                                                    | Put reusable behavior behind `Pf2eDataService` or another backend facade, then call it from the server layer                                                                                                                             |
| Search-to-storage imports        | No search modules                                                                                                                                                                                                                                         | `src/search/**/*` cannot import `src/data/**` directly                                                                                                                                          | Keep runtime search logic behind search-owned contracts and the retrieval port; data implements physical retrieval                                                                                                                       |
| Metadata field ownership         | `src/domain/metadata-field-catalog.ts`, `src/search/filters/metadata-execution.ts`, `src/data/metadata-row-projection.ts`, `src/data/backend/metadata-search-sql.ts`, and `src/server/metadata-presentation.ts` by concern                                | App, domain, server, and TUI modules cannot import search-owned metadata execution helpers                                                                                                      | Use the domain catalog for public field meaning, data projection for row hydration, data SQL modules for physical predicates, search execution helpers for normalization and matching, and server presentation helpers for MCP summaries |
...
- that canonical filter tree includes symmetric exact record-link leaves such as `linksTo` and `linkedFrom`; do not replace them with feature-local relation request shapes once a query crosses the shared boundary
- `src/domain/metadata-field-catalog.ts`, `src/domain/metadata-field-types.ts`, and `src/domain/search-filter-metadata.ts` own the shared metadata field vocabulary, public metadata field catalog, and atomic metadata predicates carried inside that contract
...
- `src/search/request-compilation.ts` and `src/search/contracts.ts` own execution-facing compiled filter shapes and the storage-agnostic retrieval port
- `src/search/filters/` owns normalization, validation, and record-level matching helpers for execution filters; it does not own public metadata field semantics, row hydration, SQL builders, or MCP presentation projection
- `src/data/backend/search-sql.ts`, `src/data/backend/metadata-search-sql.ts`, `src/data/record-queries.ts`, and `src/data/backend/search-retrieval.ts` own SQLite SQL strings, row hydration into runtime search candidates, and the concrete retrieval-port implementation
- `src/app/**`, `src/domain/**`, `src/server/**`, and `src/tui/**` must not import the search execution DTOs or compiler directly
- `src/search/**` must not import `src/data/**`; runtime search receives candidate, lexical, and semantic retrieval through the typed port
...
- keep shared discovery internals split under `src/app/search-discovery/`: applicability, cache keys, metadata-field discovery, metric discovery, value discovery, reader preparation, query builders, and the service facade are separate owners
- keep search-semantics ontology internals split under `src/app/ontology/search-semantics/`: the domain builder composes focused field, metric, pack, value-node, child-source, record-child-source, query-builder, derived-tag, type, and label owners
- keep query construction in search-semantics query builders rather than hiding canonical `SearchRequest` assembly inside node presentation code
...
- keep shared entity-page document and page-text rendering in `src/app/ontology/entity-page.ts`, keep raw/vendor page fact projection in `src/app/ontology/entity-page-facts.ts`, route relation-aware page composition through `src/app/ontology/entity-page-service.ts`, and route qualifying TUI page consumers through `services.user.entityPages` plus `src/tui/page-document/*` instead of importing the plain-line presenter directly
- treat `src/app/ontology/presenter.ts` as the durable plain-line presentation path for non-qualifying ontology/editorial consumers that still need `OntologyTextLine[]`; do not use it for structured record-page TUI routes, inline targets, or relation-aware page navigation
...
- structured-draft route callers pass semantic field or leaf intents to the route owner; TUI picker rows such as `Pf2eTerminalQueryFieldOption` remain display/session data and are not the router contract
