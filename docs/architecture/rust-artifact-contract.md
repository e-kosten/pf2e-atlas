...
| Records | `records` | Canonical normalized record identity, derived `record_family`, presentation text, source path/provenance, selected direct `system_*` projections, normalized price/time facts, variant facts, default retrieval visibility, generated search text projection, and raw JSON for parity/debugging. |
