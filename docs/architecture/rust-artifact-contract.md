...
- The `atlas-index` vector query boundary accepts an already-computed query vector and optional `SearchFilterNode`. Text-to-query-vector embedding remains owned by `atlas-embedding` and should be called by a higher search orchestration layer before it invokes vector-index SQL.
