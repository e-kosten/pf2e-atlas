...
- Runtime semantic search groups sqlite-vec unit hits by parent record before returning user-facing records. Parent units are the strongest ranking evidence; child units can recover records, but their raw vector distance is adjusted by unit kind before collapse so narrow activation blocks, titled options, and heading sections do not automatically outrank a close whole-record match. Diagnostics may expose both raw vector distance and adjusted rank distance.
