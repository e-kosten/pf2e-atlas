...
`graph links` retrieves a one-hop local graph context around one seed record. It accepts a canonical record key or strict resolvable record name, includes outgoing references by default, and includes backlinks only when the caller passes a positive backlink limit. `graph uses` is the backlinks-focused form for records that reference or use the seed. Both commands use the named product reference graph policy and authored `reference_edges` only. The default graph includes non-embedded authored edges across visibility classifications; copied embedded capabilities require the expanded mode to prevent duplicate/noisy context. Variant text lookup uses the ordinary retrieval disposition, while concrete record-key seeds may open direct-only source-instance or legacy groups. Implementation-only provenance remains inspection-only because it has no authored product meaning, and linked legacy variants remain outside ordinary ranking while directly addressable; each exclusion has a non-auth rationale. `graph variants` exposes variant siblings/progressions from variant metadata, and `graph remaster` exposes legacy/remaster relationships from `remaster_links`.
