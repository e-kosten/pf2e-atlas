...
`graph links` retrieves a one-hop local graph context around one seed record. It accepts a canonical record key or strict resolvable record name, includes outgoing references by default, and includes backlinks only when the caller passes a positive backlink limit. `graph uses` is the backlinks-focused form for records that reference or use the seed. Both commands use the named product reference graph policy and authored `reference_edges` only. The pinned base has no authentication boundary but still applies public-only default graph policy and `is_default_visible` variant filtering, so it is not GM-complete. Checkpoint A's target removes classification-only suppression. Copied embedded capabilities may require an expanded mode to prevent duplicate/noisy context, implementation-only provenance may remain inspection-only because it has no authored product meaning, and legacy variants may be demoted from ordinary ranking to prevent duplicate remaster results while staying directly addressable; each exclusion requires a non-auth rationale. `graph variants` exposes variant siblings/progressions from variant metadata, and `graph remaster` exposes legacy/remaster relationships from `remaster_links`.
...
V1 graph retrieval accepts any seed record that direct key lookup can load, including records outside default browse routing. Exposed edges and neighbor records follow the product graph policy; typed hidden/private/internal classification alone does not remove useful authored edges or create a privacy/security claim. Any omitted provenance or copied-capability edge is excluded by its explicit non-auth product rationale, which also governs counts and truncation.
