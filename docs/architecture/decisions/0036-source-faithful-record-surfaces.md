...
ADR 0023 remains authoritative for CLI JSON. CLI record-bearing commands extend the existing `atlas-record::RecordJson` projection additively and preserve its envelope, `snake_case`, detail vocabulary, one schema across hydration depths, structured content blocks, ambiguity data, raw-source opt-in, partial batch signaling, and exit classes. App DTOs and a parallel CLI semantic model are forbidden.
