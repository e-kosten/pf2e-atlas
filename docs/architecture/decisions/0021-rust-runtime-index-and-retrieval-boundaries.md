...
`atlas-index` owns runtime index read/write boundaries. Runtime read interaction goes through focused read traits such as `RecordReadIndex`, `IdentityReadIndex`, `FilterReadIndex`, `FtsReadIndex`, `VectorReadIndex`, `ReferenceReadIndex`, `VariantReadIndex`, `RemasterReadIndex`, and `DiscoveryReadIndex`, with the composite `RetrievalReadIndex` bundle for consumers that legitimately need the full retrieval read surface. `SqliteIndexReader` is the SQLite implementation. Artifact construction goes through `IndexArtifactWriter`, with `SqliteIndexWriter` as the SQLite implementation. Runtime artifact operations such as base validation, inspection, record loading, record lookup primitives, filter-backed listing, filter discovery primitives, lexical query primitives, vector validation, and vector query execution should remain behind the index read boundary rather than being opened directly by CLI or search code.
...
`atlas-search::AtlasRetrievalService` is the durable product-facing retrieval boundary. CLI and future TUI or web surfaces should route retrieval and filter discovery use cases through this service rather than constructing separate lookup, list, lexical, semantic, hybrid, or discovery services as peer public surfaces. The service owns retrieval orchestration and result assembly over the index-owned `RetrievalReadIndex` capability bundle and embedding components, while direct SQLite interaction remains inside `SqliteIndexReader`.

Consumers should depend on narrow `atlas-search` capability traits when they only need a subset of retrieval behavior: `RecordRetrieval`, `TextRetrieval`, `SimilarRetrieval`, `GraphRetrieval`, `VariantRetrieval`, `RemasterRetrieval`, and `FilterDiscoveryRetrieval`. These traits expose search-owned request/result DTOs and keep index-owned rows, SQL details, vector errors, and filter compiler internals out of product callers. Semantic-only retrieval and low-level fusion controls are expert/debug APIs rather than ordinary product entrypoints.
...
- `AtlasRetrievalService` owning retrieval and filter discovery orchestration
