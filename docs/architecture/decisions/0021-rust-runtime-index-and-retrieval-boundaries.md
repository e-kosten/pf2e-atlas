...
Rust should keep ingest transformation separate from index backend access. `atlas-ingest` builds the normalized payload for replacement artifacts, but database-specific reads and writes belong to `atlas-index` backend implementations. Runtime surfaces should consume an existing artifact through the runtime/index/retrieval boundaries.
...
`atlas-index::SqliteIndexReader` owns runtime SQLite connection lifecycle and all direct runtime read interaction with the completed SQLite artifact. Runtime artifact operations such as base validation, inspection, record loading, record lookup primitives, filter-backed listing, lexical query primitives, vector validation, and vector query execution should be methods or owned collaborators behind `SqliteIndexReader`.

Public path-based runtime operation helpers are not durable API. Passing a path to an `SqliteIndexReader::open_*` constructor is the correct boundary for opening an artifact. Passing paths to validation, inspection, search, or record-loading helpers that then open their own connections is a migration convenience that should be removed as the index boundary is consolidated.

`atlas-search::AtlasRetrievalService` is the durable product-facing retrieval boundary. CLI and future TUI surfaces should route retrieval use cases through this service rather than constructing separate lookup, list, lexical, semantic, or hybrid services as peer public surfaces. The service owns retrieval orchestration and result assembly over `SqliteIndexReader` and embedding components, while direct SQLite interaction remains inside `SqliteIndexReader`.
...
`atlas-ingest` remains the owner of source-to-index build orchestration, but it must not own direct database writes. After normalization, enrichment, and embedding generation, ingest lowers its internal state to `atlas-index::IndexBuildInput` and invokes `atlas-index::IndexArtifactWriter` implementations. `SqliteIndexWriter` writes both `document_embedding_cache` and the sqlite-vec `record_vector_index` before publishing the completed artifact; spike backends such as `LadybugIndexWriter` own their own physical write model.
...
Runtime-facing code should be refactored so path-based operation helpers become `SqliteIndexReader` methods or private CLI wrappers over runtime/index handles. The end state should not leave public helper APIs that accept a database path and open SQLite independently for validation, inspection, record reads, vector maintenance, or search.
...
- `SqliteIndexReader` owning direct SQLite artifact access
...
- `atlas-ingest` owning source-to-index build orchestration while `atlas-index` owns backend artifact read/write implementations
