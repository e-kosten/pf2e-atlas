...
`atlas search <text>` is the normal ranked text search surface. By default, text search uses hybrid retrieval over FTS and vector lanes and fuses record ranks with weighted reciprocal rank fusion. The default lane weights are equal:
...
This equal-weight default is equivalent to plain RRF, but the implementation uses one weighted-RRF path so later tuning does not introduce a second fusion engine.
...
User query text is treated as plain text, not raw SQLite FTS5 syntax. The Phase 6 baseline builds safe FTS syntax from normalized non-stopword tokens, quotes each token, joins tokens with `OR`, and does not use prefix matching. Custom synonym maps, PF2E domain token rewrites, and high-signal token derivation are out of scope for the baseline. Tokenizer, stemming, prefix, and OR-vs-AND behavior are deferred to fixture-backed follow-up tuning.
...
`atlas-search::AtlasRetrievalService` owns identity tiering, FTS/vector lane orchestration, fusion, result assembly, and search-window shaping. `atlas-index::SqliteIndexReader` owns direct SQLite access for FTS and vector retrieval. `atlas-cli` owns only argument parsing, JSON/human presentation, and exit codes.
...
Search JSON remains compact by default. Rank, score, lane, vector-unit, and query-analysis details belong behind `--explain`, where per-result explain fields are attached to each result's `match` object.
...
Search quality tuning should prefer measured changes to FTS weights, RRF weights, candidate windows, rank constants, tokenizer/stemmer behavior, and vector unit policy before adding generic rerank adjustments.
