...
Writer validation and publication share one non-serializable `ValidatedArtifactReceipt`. After SQLite handles close and the staged file is synced, companion-free, and sealed, deep validation and SHA-256 operate through one retained file identity. The receipt binds that handle, its portable Unix device/inode or Windows volume/file-index identity, path, intended target, byte count, modification token, digest, and successful validation. Ingest may carry the receipt and use its digest for the existing manifest but owns no identity or validation policy. Publication consumes the receipt, rejects drift, and avoids only redundant SHA reads of that unchanged staged/renamed file. It still performs one ordinary byte copy into the digest-named generation, syncs and independently hashes the copy, and rejects matching source/generation identities; hard links, reflinks, and other shared-mutation aliases are forbidden. Recovery and reader acquisition continue to perform their independent full verification and never accept serialized receipt evidence.
