...
Hazard provenance adds `provenance.source_metadata[]` to the hazard `RecordJson` only when provenance evidence is requested. The dedicated `atlas record provenance <hazard-key> --json` envelope exposes the same array at `data.hazard_provenance.source_metadata[]`. Each row is tagged by its stable semantic `field`, adds `save` or `entity_id` only when that identity is part of the fact, and carries the existing `HazardFact` presence, support, exact value, and `provenance.relativeSourcePath` envelope. Root and defense facts precede save facts; entity facts preserve canonical authored entity order. Malformed values remain one exact `support: "unsupported"` metadata fact and may produce one localized `availability` issue, but are not copied into `unsupported_fields`; unrelated genuine unsupported facts remain there unchanged. Ordinary record JSON omits this explicit provenance, and valid, consistent, zero, or empty provenance-only values do not create availability issues. This additive CLI projection does not add an application provenance DTO or change the persisted hazard codec, artifact schema, or manifest contract.
...
The shared typed expectedness policy classifies already-owned facts as renderable, omitted, provenance-only, or actionable before human projections consume them. This does not alter `RecordJson` keys or presence states: false and zero remain known values, base spell `Unsupported` remains distinct from Missing/Null, and exact source paths and payloads remain machine/provenance data rather than display-message inputs.
