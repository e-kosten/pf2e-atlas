...
Record-facing commands use `summary`, `preview`, `description`, `standard`, and `full` detail levels. All detail levels keep the same tagged record schema and vary only by hydration depth. `summary` is identity-oriented, `preview` adds compact typed scan concepts and truncated prose, `description` emphasizes complete descriptive content, `standard` is the normal typed entity view, and `full` adds full source metadata and supplementary content. The Rust CLI does not accept `minimal`, `compact`, or TypeScript-specific detail values. Raw source JSON is exposed only by explicit `--include-raw` opt-in, appears as top-level `source_json` in the record payload, and is independent of detail level.
...
The `unmigrated` presentation is a temporary, explicit registry boundary for non-creature families assigned to named H1-H10 plans and the H12 acceptance checkpoint. It is not a silent catch-all or final product state. Each family replaces that boundary with its own optimized variant; the unreleased CLI contract uses direct replacement without compatibility aliases, shims, feature switches, or dual serializers.
