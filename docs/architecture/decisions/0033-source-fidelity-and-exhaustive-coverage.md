...
Raw source-path inventory remains a separately versioned diagnostic surface. It can discover leaves and source drift, but its path families, wildcard normalization, warnings, or `unconsumed` rows cannot report authoritative completeness. Full-pin source prevalence is source/registry-bound metadata, not a count derived from focused evidence. Authoritative receipt sets reconcile their separately declared fixture prevalence, exact fixture identities, member uniqueness, authored order, and occurrence counts without claiming to enumerate the corpus. New, removed, or reclassified meaningful paths in a source-refresh diff remain review-blocking until their exact registry tuple and leaf ledger are updated.
