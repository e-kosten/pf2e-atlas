...
Raw source-path inventory remains a separately versioned diagnostic surface. It can discover leaves and source drift, but its path families, wildcard normalization, warnings, or `unconsumed` rows cannot report authoritative completeness. Authoritative receipt sets reconcile exact fixture identities, source prevalence, member uniqueness, authored order, and occurrence counts. New, removed, or reclassified meaningful paths in a source-refresh diff remain review-blocking until their exact registry tuple and leaf ledger are updated.
