...
- `src/search/filters/metadata-execution.ts` owns search execution behavior for metadata fields: SQL expressions, filter-value sources, value normalization, and record-value access used by search matching.
...
`src/search/filters/` no longer owns public metadata semantics. App, domain, server, TUI, and data modules must not import search-owned metadata execution helpers for field meaning or row hydration.
