...
`atlas-app-service` owns final app presentation composition through generated DTOs exported by `atlas-app-model`. `RecordSurfaceView` is the one app-facing record contract: common metadata, a profile tag, a tagged entity-specific typed presentation body, optional deduplicated user issues, one optional outer record-reference owner, and an optional exact `EncounterRuntimeView` bag. Creature, hazard, and standalone spell families have named typed bodies; other families return an explicit typed unavailable presentation until their separate migration. Encounter composition uses participant metadata beside that surface and places all current/final mechanics in the optional encounter bag. The former `StatBlockView.values[]` and generic record-section bridge are removed directly.

`atlas-record` classifies already-typed facts through one presentation-neutral `Render | Omit | Provenance | Issue` policy. App-service maps only the `Issue` disposition into capability-local common issue DTOs and collapses repeated family-domain views by typed fact identity. Optional Missing/Null/nonapplicable and known-empty values are silent; false and zero are known values; supported source-only facts remain provenance; required malformed, ambiguous, unsupported, unavailable, or populated unmodeled facts remain actionable. Messages are display-only and contain neither source paths nor opaque component IDs.

Record-detail composition requests the existing graph owner with outgoing 8/backlinks 0 by default and projects its ordered records and edges, exact totals, truncation, and typed nonfatal errors into `RecordSurfaceView.references`. A zero limit is `not_requested`, not an empty result with fabricated totals. Explicit backlink requests use the same request and projection; search-compact and encounter profiles do not read graph context implicitly. Canonical creature/hazard entity and occurrence relationships remain provenance and are absent from ordinary family relationship output, so they cannot masquerade as cross-record links.

The additive record-detail request fields are `reference_outgoing_limit` and `reference_backlink_limit`. Omitting them preserves the outgoing-8/backlinks-0 defaults, while an explicit zero requests the `not_requested` state for that direction; nonzero values use the existing graph limit validation and cap. `RecordSurfaceView.references` contains `outgoing` and `backlinks`, each tagged by `state`. An `available` section contains `requested_limit`, ordered `records` and `edges`, `total_records`, `total_edges`, and `truncated`; an `unavailable` section contains `requested_limit`, `code`, and `message`; `not_requested` contains no totals. Reference records expose `record_key`, `title`, and `kind`. Edges expose `from_record_key`, `to_record_key`, optional `display_text`, `reference_text`, and `source`; the source contains `kind`, `visibility`, and `relation_kind`. These fields are optional additions around the existing record-detail and spell-selection request behavior, not a separate endpoint or CLI grammar.

App DTO integers that cross ordinary HTTP JSON use JavaScript `number` in generated TypeScript. Every affected Rust serializer and deserializer enforces the JavaScript safe-integer range, and request path/body construction enforces the same invariant in the browser. This bounded app contract does not expose full-width `i64` as `bigint`, strings, or a client decoder layer.
...
`RecordPresentationDocument` is not an app DTO or an app-service fallback. CLI and storage-neutral presentation contracts remain owned by `atlas-record`; app-service projects `RetrievedRecord` directly into `RecordSurfaceView`. Creature, hazard, and standalone-spell kinds use their typed bodies, while other kinds remain behind explicit typed unavailable boundaries until their separately approved family cutovers.

Standalone spell and ritual bodies expose a compact backend-ordered form catalog plus one mandatory effective form for the requested opaque form ID and cast rank. App-service consumes the canonical resolver result once and projects classification, casting, targeting, defense, damage, duration, heightening, ritual, rules, and field-local unavailable states from that result. Authored patch keys and a duplicate base definition are not app DTOs. Semantic damage and fixed-heightening changes use unique typed member association; ambiguous association is shown as a neutral effect rather than guessed or exposed as an internal key. Hazard provenance carries the approved source-only metadata as tagged typed facts with exact values and source context, while ordinary hazard fields and issues remain unchanged.
...
Encounter participant views attach runtime controls to named semantic fields such as vitals and conditions. Adjusted values render final values inline, with modifier explanations available through tooltip or popover details. Canonical mechanics that have final/current runtime counterparts are omitted from the encounter-profile creature body rather than serialized twice. The same encounter body retains consumer-ready canonical context that has no runtime counterpart: IWR and hardness, AC context without static AC, authored save notes/details without static save modifiers, and senses/languages/details without static perception. Runtime activities carry the canonical traits from their exact typed occurrence directly, so consumers need neither a second fetch nor a static/runtime join.

Activity and authored embedded spell content is backend-composed for record-detail and encounter-participant profiles from typed occurrence and exact actor-local target associations. Static and encounter spellcasting use ordered entry and occurrence rows, preserve repeated authored occurrences, and expose creature-parent spells in explicit standalone collections; encounter rows also carry final attack/DC/slots and spell rows do not remain in generic runtime activities. Claimed content appears on its owning row exactly once and is absent from general content, while unrelated embedded capability content remains general. The UI renders the ordered typed blocks, reference targets, and runtime mechanics it receives; it does not parse Foundry HTML, infer associations, join public owner maps, or duplicate content into overview sections.

Record kinds without a creature, hazard, or spell body return the explicit unavailable presentation until their own typed bodies are deliberately designed.
