...
During live query edits, the filter explorer keeps the current model mounted and treats displayed counts as the snapshot for the active editing session. Query edits update the host query immediately, clear cached count models, and schedule a debounced same-mode refresh with stale-result protection. The visible picker remains interactive with old counts while the refresh runs; refreshed counts merge back without allowing older completions to replace newer query state. Discovery-mode changes and reopened explorer sessions may also load fresh matching or catalog counts. This keeps picker input responsive while avoiding synchronous count recomputation on the Ink event loop for each edit.
