...
9. `atlas-app-service` holds retrieval state for application workflows, adapts app DTOs into `atlas-search` requests, and composes canonical facts and occurrences with local runtime state. Encounter composition produces one `EncounterRuntimeView` with named semantic fields and typed provenance. It is the final static/runtime composition point, not a web-only service.
