...
Canonical Spell and Consumable bodies remain attached to `RetrievedRecord` across keyed lookup, ranked search, detail, and graph results. Consumable attachments on NPCs, Characters, and hazards travel in the same aggregate. CLI JSON projects standalone consumables and NPC/hazard occurrences from those typed owners; Character occurrences remain persisted and hydrated for the later Character family without becoming an H5 public surface. Intrinsic definition and maximum values stay separate from read-only authored quantity, current uses, current HP, container, and equipped state. Exact resolved-target differences remain typed evidence beside the target rather than replacing it or creating a second Spell model.

CLI JSON projects Spell bodies as the tagged `spell` presentation, including typed source-presence facts, authored rich content, ritual and heightening data, and backend-ordered forms whose identifiers are opaque to consumers. Terminal presentation consumes the same projection. App record detail accepts an opaque backend-provided spell form ID only when paired with a cast rank; app-service maps that pair to server-owned overlay context, invokes the canonical typed form resolver once, and returns one mandatory effective definition or localized unavailability beside a compact human form catalog. The app contract does not expose a second base definition or authored patch tree, and consumers do not reconstruct labels or effects. Spell and consumable FTS and semantic embedding inputs derive from canonical bodies and approved owned public content and references; they do not reconstruct meaning from query rows or raw source JSON.
