...
- `atlas-local-state` owns durable mutable local state stored outside the generated artifact, including saved-list schema/items, encounter schema/participants, participant creation baselines and atomic resets, and participant-local spell-resource ledgers.
...
Ordinary artifact use is deliberately cheap: reader acquisition validates the canonical manifest digest and sealed local generation identity, checks the manifest envelope plus exact artifact-contract/schema stamps, and keyed hydration validates only requested rows. Matching trusted generations avoid whole-file hashing; untrusted or changed cache entries are handled at an explicit recovery boundary before immutable SQLite open. Final validation uses one embedded production artifact and checks schema and required-table shape, foreign keys, logical/cross-table invariants, global FTS/catalog/vector coverage, canonical body coverage, and bounded public-reader hydration. It does not claim a physical SQLite page or B-tree scan. Whole-corpus canonical-body decode/re-encode and projection reconciliation are optional diagnostics rather than builder or CI gates. The artifact contract version is the umbrella for incompatible codec/projection/hydration semantics, while schema and manifest versions independently cover physical DDL and envelope shape. Changes to migrations, persisted codecs, manifest fields, or reader/writer interpretation receive explicit human compatibility and rebuild review alongside the runtime rejection tests.

Canonical Spell bodies remain attached to `RetrievedRecord` across keyed lookup, ranked search, detail, and graph results. CLI JSON projects that body as the tagged `spell` presentation, including typed source-presence facts, authored rich content, ritual and heightening data, and backend-ordered forms whose identifiers are opaque to consumers. Terminal presentation consumes the same projection. App record detail accepts an opaque backend-provided spell form ID only when paired with a cast rank; app-service maps that pair to server-owned overlay context, invokes the canonical typed form resolver once, and returns one mandatory effective definition or localized unavailability beside a compact human form catalog. The app contract does not expose a second base definition or authored patch tree, and consumers do not reconstruct labels or effects. Spell FTS and semantic embedding inputs derive from the canonical body and its owned public content and references; they do not reconstruct meaning from query rows or raw source JSON.
...
flowchart TD
    ...
    indexWriter --> artifactDb["SQLite artifact<br/>pf2e-atlas-artifact/v9"]
...
2. Ingest parses each original serialized Source document once into one crate-private recursive source tree before any `serde_json::Map` projection. That tree preserves object entry order and duplicate keys with their corresponding values, array order and multiplicity, and exact scalar/presence states. Family DTO builders apply explicit duplicate policy: the canonical spell owner retains repeated keyed entries, duplicate typed scalars, and exact unsupported evidence inside a standalone spell's `system` subtree or a consumable child's exact `system.spell.system` subtree (plus its typed image), except that `description.gm` remains owned by shared content extraction; the canonical hazard owner retains repeated damage entries only for an actual melee child's exact `damageRolls` map. Journal and roll-table owners retain four-state parent and child facts, exact dynamic metadata objects, ordered child identity, and result/page-local unsupported evidence. Their dispatch and identity duplicates fail closed; a duplicate nonidentity TableResult member localizes to that result so valid siblings remain available. Dispatch identity/type duplicates, duplicate `description.gm` content, spell root or child-envelope metadata duplicates, prefix-neighbor paths, non-melee hazard damage, and all other unowned duplicates fail closed. The checked legacy JSON projection serves the common record envelope/provenance/content/taxonomy paths plus mechanics and metrics for families without a canonical typed body; those consumers reject duplicates or receive values only after the typed family owner has retained the omitted evidence. Standalone spells, journals, and roll tables never populate generic item mechanics or record metrics. Ingest then records exhaustive coverage, normalizes canonical entities and occurrences, parses rich content once into `RichDocument`, resolves rich-content references, derives traits, non-canonical-family generic metrics, and aliases, generates source-backed records, runs build-time embedding work, and prepares `IndexBuildInput`. The artifact writer derives creature and hazard query metrics and spell filter rows directly from their matching canonical bodies; embedding input uses the same family-owned canonical presentation and unique owned-content sources.
...
9. `atlas-app-service` holds retrieval state for application workflows, adapts app DTOs into `atlas-search` requests, and composes canonical facts and occurrences into `RecordSurfaceView` profiles. Common metadata, deduplicated user-relevant issues, and record-detail references are separate from the tagged entity body. The issue projection consumes the `atlas-record` expectedness result over already typed facts: optional absence and known-empty values omit, provenance-only values remain provenance, and unsafe or populated unmodeled values become localized issues without paths or opaque IDs in messages. Record detail alone requests the existing graph service and projects its deterministic records, edges, totals, truncation, and typed nonfatal failures into the outer reference owner; its default is eight outgoing records and backlinks not requested. Search-compact and encounter profiles do not perform an implicit graph read. Internal entity/occurrence membership remains canonical provenance and never becomes a human record link. Creature and hazard bodies use named typed objects for zero-or-one domains and arrays only for genuinely repeated entities; families without an owned projection use an explicit unavailable boundary. Hazard surfaces are projected only from the canonical `RecordBody::Hazard` and central presentation/content owners, preserve authored occurrence order and identity, and expose image, license, and approved source-only metadata only in explicit typed provenance. Journal and roll-table surfaces retain authored child order and expose opaque server-owned child locators beneath the parent route. Their rich text contributes to the parent record's FTS row and may carry child-specific reference context, but search results remain parent-record hits. Media locations are provenance-only and are never fetched or rendered. A roll-table surface also exposes a backend-derived stateless Roll capability only when its exact `1dN` formula and complete typed result-range inventory are safe. `atlas-app-service` owns entropy and returns the sampled total plus every authored-order range match; the browser never parses formulas, supplies a total or seed, filters `drawn`, or persists history. Activity and spell presentation resolves typed occurrence targets and attaches matching occurrence-owned plus exact actor-local entity-owned content once, removing claimed documents from overview/lore; association ambiguity fails the affected row closed. Standalone spell detail carries a compact backend-owned form catalog and one mandatory effective definition produced from the requested form/rank resolver result; semantic damage and heightening labels use retained typed member associations, while unavailable fields remain explicit. Static spellcasting and encounter spellcasting are backend-composed ordered trees of canonical entries and occurrence rows; static and runtime creature-parent spells use explicit `standalone_spells`, while runtime rows additionally carry final mechanics and do not remain in generic activities. Repeated authored spell occurrences remain distinct even when they share a canonical target. Runtime spell rows expose backend-composed cast availability through a closed spend-target union: prepared slot, spontaneous rank pool, innate occurrence use, focus pool, or stateless at-will. App-service derives ownership only from canonical typed occurrence relationships, initializes supported current counts once into participant-local state, validates every mutation against the current canonical owner, and fails an affected row closed when identity, ownership, or current availability is unavailable. `EncounterParticipantView.record_view` carries that record projection; encounter participants omit duplicated static mechanics and attach the exact `EncounterRuntimeView` as its optional runtime bag, with named semantic fields, typed provenance, and concise automation limitations identified by stable codes and typed participant/condition/activity/spellcasting placement targets. It is the final static/runtime composition point, not a web-only service.
...
- Keep durable mutable local state in `atlas-local-state`, not in generated artifact tables. `LocalStateStore` owns database lifecycle and feature handles such as `saved_lists()` and `encounters()`, while feature modules own product behavior over their rows. Encounter spell-resource rows use typed participant-local ownership keys and retain both their initialization baseline and current remaining value; they are not reconstructed from display labels. New participants capture their mechanical reset baseline transactionally at creation. Legacy rows without that baseline remain explicitly reset-unavailable rather than being rebaselined from migration-time or canonical state. Cross-layer workflows that need both active artifact records and local state belong in `atlas-app-service`, not in `atlas-runtime` or CLI command code. Artifact rebuilds must not be responsible for preserving saved lists, encounters, or future user-authored local rows.
