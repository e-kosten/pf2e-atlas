...
Authoritative source coverage is an ingest-owned, test-evidence contract under `contracts/source-leaf-coverage/v1`. A declaration is bound to the pinned source contract/commit/signature and one exact tuple from the authenticated 313-entry `contracts/pf2e-type-registry.yaml`; it then owns one normalized scalar, ordered array-member, or identity-retaining map-member leaf. Object and collection containers, recursive paths, and prefixes are forbidden. `[]` uses independent ordered member receipts, and `*` is valid only for a true map with an enforced key policy. The sealed accessor registry loads each fixture blob from the pinned Git tree, resolves its source path and record key, and derives mutation evidence by rerunning the production typed parser and selected DTO, canonical, post-build-input, hydration, and public projection functions. Callers cannot implement accessors, construct receipt builders, provide source bytes or owner values, or self-assert success. Full-pin `source_prevalence` metadata is source/registry-bound and remains independent from the focused `fixture_prevalence` receipt set. Receipts compare the shared DTO `SourcePresence` state, type, value, member identity, order, and multiplicity through each declared final owner. Declarations, diagnostics, transient metrics, raw clones, and caller-supplied success booleans cannot stand in for those receipts.
