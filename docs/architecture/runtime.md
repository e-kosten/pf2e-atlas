...
| `atlas-local-state` | Durable mutable local state stored outside the generated artifact, including saved-list schema/items, encounter schema/participants with snapshots for unresolved record-backed rows, creation-captured reset baselines, atomic participant reset, and typed participant-local spell-resource baselines/current values. | Generated artifact schema, source ingest, retrieval/search semantics, runtime path policy, CLI presentation, or frontend state. |
...
| `atlas-ingest` | Source loading, the versioned serialized PF2e Source boundary, exhaustive coverage declarations, Foundry-specific parsing, canonical normalization, non-creature generic metric/side projections, generated records, aliases/remaster links, reference resolution, application of the shared retrieval disposition, embedding execution during builds, and owned conversion into `IndexBuildInput`. | Prepared Foundry Data as a contract, runtime raw-JSON fallback, public embedding-specific API, runtime query orchestration, CLI presentation, broad crate-root behavior, metric-definition ownership, physical SQLite writer ownership. |
...
Source normalization emits ingest construction facts beside each normalized record. These facts retain the complete versioned NPC Source envelope and carry the canonical creature body into the atomic artifact writer, source identity such as slugs and compendium-source locators, embedded item identity/provenance/content references, and journal page content parsed from Foundry source JSON. The typed envelope preserves valid sibling fields and the immutable provenance/audit payload without becoming a raw semantic fallback. Query metric rows are one-way projections from `CreatureRecord`; creature actor/activity/spellcasting mechanics are not separately stored or hydrated. Later ingest phases use construction facts for aliases, remaster links, and source-backed generated records instead of reparsing `AtlasRecord.raw_json`; reference, FTS, and embedding projections consume the normalized `RichDocument` outputs produced during normalization. `atlas-record::ProductRetrievalPolicy` is the one storage-neutral decision owner for ordinary, direct-only, and inspection-only routing; ingest uses it for embedding eligibility and index writing persists the same decision. Persisted raw JSON remains provenance/debug input and a future analysis substrate, not the normal construction API between ingest phases.
