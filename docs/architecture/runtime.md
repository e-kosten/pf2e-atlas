...
`atlas-record::project_creature_mechanics` exposes the same canonical creature body as storage-neutral mechanics base facts. It covers defenses, awareness and skills, movement, resource maxima, strikes, actions, spellcasting entries, spells, rolls and DCs, damage, action costs, frequencies, uses, spell slots, and actor ritual DC. Targets are composed from typed component and occurrence identities; labels, metric-key prefixes, and prose never determine canonical target identity. Unsupported source shapes and capabilities remain explicit structured projection values associated with their target or occurrence. This projection contains source/base facts only: mutable current values, turn state, and action budgets remain outside the canonical artifact and belong to app-service runtime policy.
