...
The local web filter editor is an app-layer projection over product discovery, not a frontend-owned field catalog. `atlas-app-service` adapts `atlas-search` discovery into `FilterEditorView`, including app field ids, groups, typed controls, placement, labels, supported operators, count support, metric comparison controls, and display ordering such as rarity domain order. `web/atlas-ui` renders that contract and may keep local interaction state, but it should not hard-code product filter groupings, control kind, fallback value lists, or field label policy. Actual frontend filter values should be stored as app-model `FilterClause` entries so visible controls, URL state, search execution, and discovery context share one clause-first representation.
