...
Construction-time writes are separate and go through `IndexArtifactWriter` implementations such as `SqliteIndexWriter`, which write a temporary artifact only after canonical records/entities/occurrences/relationships/content, ordered record mechanics children, projections, embedding cache rows, and `record_vector_index` are complete. Once all SQLite handles close, the writer syncs and seals the staged file, validates and hashes one retained open-file identity, and returns an opaque single-use receipt. Ingest uses that receipt's digest for the unchanged adjacent manifest and passes the live receipt into the index-owned pair publisher. The publisher uses a bounded exclusive-lock acquisition, preserves independent recovery verification, creates and verifies one byte-copy generation with a distinct portable identity, avoids only redundant staged/visible full-file hashes, preserves the prior matching pair in fixed recovery backups, and restores or recovers that pair after publication failure or interruption. Product surfaces route retrieval and filter discovery through the CLI client/app-service boundary, `atlas-runtime`, and `AtlasRetrievalService`; they do not open SQLite or assemble retrieval dependencies directly.
