...
flowchart TD
    ...
    localStateDb["SQLite local state<br/>saved lists and mutable local data"]
    ...
    cli --> localState["atlas-local-state<br/>saved lists and mutable local state"]
...
| `atlas-app-service` | Long-lived interactive workflow orchestration over `atlas-runtime`, `atlas-search`, and `atlas-local-state`, including full retrieval-service startup, result-window metadata, filter editor projection, app filter lowering, record detail projection, saved-list read projection/hydration, and app error mapping. Its service entry module owns the public handle, shared state, local-state path, and bounded retrieval executor; workflow modules own result-window, record-detail, filter-discovery, and saved-list orchestration plus their focused tests. | Direct `atlas-index` access, SQLite reader assembly, no-embeddings retrieval shortcuts, HTTP routing, frontend layout/state, or a single unbounded global request lane. |
...
| `atlas-local-state` | Durable mutable local state stored outside the generated artifact, including saved-list schema, migrations, item ordering, and display snapshots for unresolved saved rows. | Generated artifact schema, source ingest, retrieval/search semantics, runtime path policy, CLI presentation, or frontend state. |
...
Durable mutable state does not live in the generated artifact. `atlas-local-state` owns local-state schema and product APIs, starting with saved lists. `atlas-runtime` only resolves the local-state database path beside the active artifact; it does not own saved-list behavior. Saved-list item additions must resolve to one active canonical `RecordKey`; local-state accepts already-resolved saved-list item inputs and stores canonical keys plus snapshots. App-service owns workflows that resolve record refs, call local-state, and hydrate active artifact records. Saved rows are preserved if a later artifact rebuild no longer contains that key. Product surfaces should hydrate saved-list records from the active artifact when possible and mark unresolved items explicitly, using stored snapshots for display.
