...
The local web surface follows the same boundary through `atlas-app-service`. `atlas web` resolves path overrides and starts Axum; `atlas-web` serves the embedded built frontend plus `/api/*` routes, but app-service retrieval workers open full `AtlasRetrievalService` instances with `AtlasRuntime::open_retrieval_service`. Web startup fails if the full semantic-search runtime is not ready; the app service must not call `open_retrieval_service_no_embeddings`, which is reserved for short-lived CLI workflows that intentionally do not need semantic retrieval. Axum handlers call the cloneable app-service handle, and the app service owns result-window metadata plus a bounded retrieval executor so read-only requests from multiple clients do not serialize behind one global database lane. Queue saturation is reported as a retryable service-busy app error rather than growing an unbounded backlog. Saved-list web reads are app-service workflows: app-service opens the runtime-resolved local-state database, reads saved-list rows through `atlas-local-state`, hydrates active record keys through the retrieval executor, and marks unresolved rows explicitly.
...
Durable mutable state does not live in the generated artifact. `atlas-local-state` owns local-state schema and writes, starting with saved lists. Saved-list item additions must resolve to one active canonical `RecordKey`, but saved rows are preserved if a later artifact rebuild no longer contains that key. Product surfaces should hydrate saved-list records from the active artifact when possible and mark unresolved items explicitly, using stored snapshots for display.
