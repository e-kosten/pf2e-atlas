...
| --- | --- | --- |
| `atlas-app-model` | Interactive app workflow DTOs, app errors, readiness views, filter editor contracts, basic filter state, result-window request/response types, the common record-surface metadata/profile wrapper, tagged entity-specific typed presentation bodies, the optional exact encounter-runtime bag, serde contracts, and generated TypeScript app contracts. | Durable search semantics, SQLite access, runtime path policy, CLI presentation, generic section/value registries, target-string stat bags, or duplicate canonical record/rich document logic. |
| `atlas-app-service` | Application workflow orchestration over `atlas-runtime`, `atlas-search`, and `atlas-local-state`, including web full retrieval-service startup, explicit on-demand retrieval modes for short-lived local CLI clients, result-window metadata, filter editor projection, app filter lowering, record-surface profile selection/density/omission, saved-list read projection/hydration, encounter runner projection/hydration, typed canonical/runtime composition, and app error mapping. Its service entry module owns the public handle, shared state, local-state path, and bounded retrieval executor; workflow modules own result-window, record-detail, filter-discovery, saved-list, and encounter orchestration plus their focused tests. | Direct `atlas-index` access, SQLite reader assembly, public target-string or string-key semantic lookup, generic section/value composition, implicit retrieval fallback policy, HTTP routing, frontend layout/state, or a single unbounded global request lane. |
...
| `atlas-local-state` | Durable mutable local state stored outside the generated artifact, including saved-list schema/items, encounter schema/participants with snapshots for unresolved record-backed rows, creation-captured reset baselines, atomic participant reset, typed participant-local spell-resource baselines/current values, and captured Active/Disabled hazard state. | Generated artifact schema, source ingest, retrieval/search semantics, runtime path policy, CLI presentation, or frontend state. |
| `atlas-record` | Storage-agnostic canonical records/entities, contextual occurrences, typed metric definitions and labels, canonical mechanics targets/facets and source-fact projections, `RichDocument`, rich-content renderers, product retrieval disposition, reference graph policy, reference traversal, section-tree projection, canonical-mechanics FTS/embedding presentation projection, presentation profiles, and the shared-base/tagged-entity CLI `RecordJson` projection with presence-based detail hydration. | Foundry HTML/macro parsing, SQLite names, validation diagnostics, top-level CLI envelopes, app/web DTOs, embedding model execution. |
| `atlas-ingest` | Source loading, the versioned serialized PF2e Source boundary, exhaustive coverage declarations, Foundry-specific parsing, canonical normalization, generic metric/side projections for families without a canonical body, generated records, aliases/remaster links, reference resolution, application of the shared retrieval disposition, embedding execution during builds, and owned conversion into `IndexBuildInput`. | Prepared Foundry Data as a contract, runtime raw-JSON fallback, public embedding-specific API, runtime query orchestration, CLI presentation, broad crate-root behavior, metric-definition ownership, physical SQLite writer ownership. |
...
Those profiles share one `RecordSurfaceView` outer contract: common record/source metadata, a profile tag, a tagged entity-specific presentation body, and an optional encounter bag. `EncounterParticipantView.record_view` names the participant's record projection. Creature and hazard bodies expose zero-or-one domains through optional named typed objects and use arrays only for true-many domains; genuinely known-empty collections are omitted from serialized surfaces and represented as optional in generated TypeScript, while populated collections remain ordered arrays. A selected creature domain whose canonical root or required nested member is missing, null, unsupported, ambiguous, failed, or unsafe instead appears in the body's typed `unavailable_domains` sidecar. A hazard surface carries equivalent field-level Missing, Null, and Unsupported states in its typed `unavailable_fields`, is projected exclusively from `RecordBody::Hazard`, and retains authored occurrence identity/order, owned rich content, typed rules, and relationships. Hazard image and publication-license facts remain explicit provenance only and are not rendered as content. These sidecars are display-only and must never be parsed for behavior. Unsafe individual members are not exposed as valid values, while their typed cause prevents omission from masquerading as known-empty. Activity and authored embedded spell content is nested on its matching static or encounter row as ordered typed `RichDocument` blocks: app-service resolves typed occurrence targets, combines matching occurrence-owned and exact actor-local entity-owned documents once, and excludes claimed documents from overview/lore. Repeated spell occurrences retain independent identity and authored order even when their canonical target repeats; valid creature-parent spells remain explicit with content under static and runtime `standalone_spells`, while unrelated embedded capability content remains general. Rich-content paragraphs expose Foundry checks as typed inlines with display fallback and optional statistic/DC while preserving authored paragraph/divider order. Encounter runtime activities also carry the canonical traits from the same typed occurrence. Encounter spellcasting nests ordered canonical entry identity and final attack/DC/slots with ordered occurrence spell rows, optional canonical targets, typed content, occurrence activity mechanics, and typed cast availability. A cast spend target is one closed ownership variant: prepared occurrence slot, spontaneous entry/rank pool, innate spell occurrence use, focus resource pool, or stateless at-will. Supported canonical current facts initialize participant-local resource rows once; missing or malformed current facts expose typed unavailable state, while duplicate or ambiguous ownership fails only the affected spell closed. Cast and restore mutations validate the requested occurrence and spend target against the current canonical typed association before changing one participant-local resource. No daily-rest or refocus reset rule is inferred. Generic runtime activities exclude spell rows. Missing, duplicate, ambiguous, dangling, multi-parent, or multi-match associations fail only the affected activity, spell, or entry closed. Profile density is encoded by presence or omission in app-service, not by `sections[]`, `section_order`, or string-key lookup. Encounter-participant record views keep canonical owned content, provenance, IWR/hardness, AC context, save notes/details, and senses/languages/details, but omit canonical values represented by the attached final/current `EncounterRuntimeView`, so one response does not duplicate base and adjusted values or require another fetch/client join. Disabled hazards remain visible, are excluded from turn rotation, retain independently available activities, and are not auto-advanced when their typed state changes. Standalone spell detail is projected from `RecordBody::Spell`, with opaque server-owned form identity and validated cast rank; families without one of the three canonical owners use the typed unavailable boundary. App DTO integers crossing ordinary HTTP JSON serialize as JavaScript-safe numbers, are emitted as TypeScript `number`, and reject values outside the safe-integer range at both Rust serde and browser request boundaries; full-width integer strings, `bigint`, and decoder shims are not part of this bounded contract.
Spell record detail carries one mandatory effective-form result for the default or explicitly requested opaque backend-provided form ID paired with a cast rank. App-service validates the pair, resolves its server-owned overlay context, and calls the canonical spell resolver once rather than exposing overlay locators, a duplicate base definition, or authored patches to the client.

The exhaustive registry covers every discovered document/type/role/parent-context tuple, including registration-only and provenance-only entries. Discovery and assignment never filter by visibility. Atlas currently has no authentication boundary. Search projections and product retrieval use the explicit retrieval disposition: useful authored records and content remain ordinary regardless of typed visibility classification, while tooling, linked legacy records, generated source instances, and copied embedded prose retain specific non-auth routing or duplicate-control dispositions. Visibility, role, source kind, and provenance remain typed inputs for meaning and a future separately approved auth feature; they make no current security claim. Later product projections remain staged, and every retained exclusion requires explicit non-auth product rationale and audit evidence.
...
Candidate validation retains one private in-process `SourceLoad` for one embedded
production artifact build. A checksum-bound snapshot records source/candidate and
artifact identity, structural/global validation, bounded selected-record smoke,
and timings; it cannot deserialize back into `SourceLoad` or `IndexBuildInput` and
is not available to runtime, setup, search, API, or UI code. Candidate/source or
...
rusqlite connections serve bounded keyed hydration and one structural/global pass;
...
The raw-path inventory remains an explicitly requested diagnostic rather than an
authoritative final gate. Normal source loading still performs strict per-record
admission and reports malformed shapes with record and source context.
...
The source loader deserializes each original record byte stream exactly once into the family-neutral `atlas-ingest::source::dto::value` tree. Its ordered object-member sequence retains repeated keys with distinct values; arrays retain order and multiplicity; null, empty collections, zero, false, and scalar values remain distinct. Unique record identity and type select the family policy before any legacy JSON view is created. Spell parsing owns duplicate evidence only inside the standalone `system` or exact consumable `system.spell.system` subtree (plus typed image), except for `description.gm`, which shared content extraction still owns; hazard duplicate damage admission requires the indexed child to have one `melee` type. Values admitted for omission reach canonical typed or exact unsupported owners first. Duplicate `description.gm`, spell root/child-envelope metadata, prefix-neighbor paths, non-melee hazard damage, all other unowned duplicates, and duplicate identity/type members fail explicitly. The checked `serde_json::Value` projection serves common record envelope/provenance/content/taxonomy/mechanics/metrics parsing, NPC conversion, hazard fields outside melee damage, and non-family generic normalization; it cannot silently choose a first or last duplicate and is not a semantic or provenance substitute for the typed owners.
...
`atlas-record::project_creature_facts` and `atlas-record::project_hazard_facts` are the storage-neutral projection owners for migrated creature and hazard query metric rows. The artifact writer invokes the matching projection directly on each canonical body, and canonical validation invokes the same function to verify persisted order, count, and digest. Missing and null canonical values produce no scalar row, numeric zero remains a metric value, and unsupported numeric shapes remain diagnosed canonical facts rather than coerced metrics. Hazard detection DC, broken threshold, and complex-hazard Stealth initiative binding remain non-persisted guarded conveniences owned by `project_hazard_conveniences`; query metric persistence contains authored facts only. Ingest raw metric specs remain only for record families without a canonical body.

`atlas-record::project_creature_mechanics` exposes the canonical creature body as storage-neutral mechanics base facts. It covers defenses, awareness and skills, movement, resource maxima, strikes, actions, spellcasting entries, spells, rolls and DCs, damage, action costs, frequencies, uses, spell slots, and actor ritual DC. Targets are composed from typed component and occurrence identities; labels, metric-key prefixes, opaque-ID parsing, and prose never determine canonical target identity. Structured target identities use one versioned, hex-component encoding whose closed variant tag and escaped UTF-8 components make the public string identity injective without a compatibility identifier. Unsupported source shapes, capability notes, and unsupported-only Lore or Equipment occurrence notes remain explicit structured projection values associated with their target or occurrence. This projection contains source/base facts only: mutable current values, turn state, and action budgets remain outside the canonical artifact and belong to app-service runtime policy. Production FTS, embedding parent presentation, and encounter composition consume this projection directly. `AtlasRecord.mechanics` remains only for record families without a canonical creature, hazard, or standalone-spell body. Normalized and hydrated records in those three families leave it empty; the writer rejects retained generic mechanics or metrics instead of preserving a second source of meaning.

Encounter projection consumes those typed targets only at the construction boundary. The serialized `EncounterRuntimeView` has optional named fields for zero-or-one semantics and typed collections only for genuinely repeated domains. Its outer `level` field is a `RuntimeNumberView`; contextual adjustment remains represented by that value's `base_value`, `adjusted_value`, modifiers, suppressed modifiers, and provenance rather than by the outer property name. Numeric, distance, count, roll, formula, resource, spellcasting, activity, action-budget, and condition values retain base/final state, applied and suppressed adjustments where applicable, and tagged source/canonical-target provenance. The only public incomplete-automation surface is a concise typed limitation with a stable code, a typed participant/condition/activity/spellcasting placement target, and a display-only message. Malformed, duplicate, unsupported, unmapped, raw JSON-path, publication, null, and source-noise details remain internal projection diagnostics and do not enter API JSON or generated TypeScript. Critical missing or unsafe data fails closed or leaves its typed value unavailable instead of becoming a limitation. Canonical target identity is provenance or internal diagnostic metadata; ordinary app and transport consumers never parse or compare it to select a semantic value.

Source normalization emits ingest construction facts beside each normalized record. These facts retain the complete versioned NPC, hazard, and spell source envelopes and carry the matching canonical bodies plus local consumable spell children into the atomic artifact writer. The typed envelope preserves valid sibling fields and immutable provenance without becoming a raw semantic fallback. Creature and hazard metric rows and spell filter rows are one-way projections from their canonical bodies; none is a presentation or hydration source. Spell range bodies persist original authored text only, while the bounded numeric range is recomputed into query rows under the named range-derivation rule. Later ingest phases use construction facts for aliases, remaster links, and source-backed generated records instead of reparsing `AtlasRecord.raw_json`; reference, FTS, presentation, and embedding projections consume typed bodies and normalized `RichDocument` outputs. Persisted raw JSON remains provenance/debug input and a future analysis substrate, not the normal construction API between ingest phases.

Public readers hydrate canonical Spell bodies and their owned content and reference occurrences into `RetrievedRecord`; keyed lookup, search, detail, and graph orchestration preserve that aggregate instead of substituting relational query rows. `atlas-record` derives the tagged Spell `RecordJson`, terminal facts, FTS document, and semantic embedding input from that typed body. Ordinary presentation keeps authored range text and excludes the numeric derivative, image/license provenance, and source-only diagnostics. Explicit provenance presentation may expose exact source context, image, license, and unsupported facts. Spell forms remain base definitions plus authored patches in storage; presentation resolves them through opaque `SpellFormId` values and backend-owned labels/order, with typed field-local unavailability when a form cannot be resolved safely.

`atlas index audit-source-paths` is the explicit offline diagnostic for that analysis substrate. It scans Foundry source packs and inventories normalized JSON leaves, source states, counts, and representative examples. The report is explicitly non-authoritative: every discovered leaf remains `unconsumed`, strict mode fails closed, and the inventory cannot name a product owner or certify completeness. This command may inspect broad raw source JSON because it is reporting/debug tooling; runtime lookup, search, filtering, and presentation still use typed records, side tables, content documents, and product DTOs.

Authoritative source coverage is an ingest-owned, test-evidence contract under `contracts/source-leaf-coverage/v1`. A declaration is bound to the pinned source contract/commit/signature and one exact tuple from the authenticated 313-entry `contracts/pf2e-type-registry.yaml`; it then owns one normalized scalar, ordered array-member, or identity-retaining map-member leaf. Object and collection containers, recursive paths, and prefixes are forbidden. `[]` uses independent ordered member receipts, and `*` is valid only for a true map with an enforced key policy. The sealed accessor registry loads each fixture blob and pack manifest from the pinned Git tree, derives document class, raw type discriminator, role, and parent context, requires that raw selector to equal the ledger selector, resolves its record key, and derives mutation evidence by rerunning the production typed parser and selected DTO, canonical, post-build-input, hydration, and public projection functions. Callers cannot implement accessors, construct receipt builders, provide source bytes or owner values, or self-assert success. Full-pin `source_prevalence.record_count` must equal the exact authenticated registry tuple's `corpus.count`; `occurrence_count` must equal the exact entry in the digest-authenticated immutable leaf inventory. Those global values remain independent from the focused `fixture_prevalence` receipt set. Receipts compare the shared DTO `SourcePresence` state, type, value, member identity, order, and multiplicity through each declared final owner. Declarations, diagnostics, transient metrics, raw clones, arbitrary counts, and caller-supplied success booleans cannot stand in for those receipts.
...
flowchart TD
    ...
    doc --> presentation["RecordJson / terminal projection<br/>CLI-owned shape"]
    doc --> appSurface["RecordSurfaceView content<br/>typed app profile"]
    ...
    presentation --> parentEmbedding["Embedding parent unit<br/>canonical presentation plus unique owned content,<br/>including occurrence-owned capability content"]
    tree --> childEmbedding["Overflow-only embedding child units<br/>for impacted eligible rich-content groups<br/>under the shared selection policy"]
    refs --> edges["reference_edges<br/>source_kind + visibility + relation_kind"]
    fts --> recordsFts["records_fts<br/>weighted lexical search"]
```

The durable source of authored rich text is `RichDocument`, not stripped text and not raw Foundry markup. `RichDocument` preserves HTML elements and Foundry enrichments together; plain text, structured presentation content for CLI `RecordJson`/terminal output, structured FTS rows, semantic chunks, references, app DTOs, and UI blocks are projections. `atlas-record` owns the structured heading/paragraph/list/table/divider and inline projection; Foundry Check nodes retain the same display, including structured DC, across plain, markdown, and structured outputs. Content attaches to typed record/entity/subdocument owners and retains stable content, section, and occurrence targets.
...
`RichDocument`; they do not retain and reparse source markup. Schema v4 persists the complete
...
Generated-affliction construction uses an explicit `canonical | source_instance` role and three typed host-instance-canonical relationships. Canonicals own deduplicated user-facing meaning; source instances preserve exact host occurrence/provenance. The role is construction-time duplicate-control and provenance metadata, not an authorization or visibility classification. Schema v4 persists and validates that role plus its explicit duplicate-control retrieval disposition; D1 keeps canonicals in ordinary search while source instances remain directly addressable and graph-reachable, and D3 audits the one-canonical/one-instance/three-relationship fixture before UI work.

D1 graph and backlink behavior keeps typed source kind, visibility, role, provenance, and relation kind while removing classification-only suppression. The default mode includes authored non-embedded edges across visibility classifications. Copied embedded capability edges remain outside the default mode to avoid duplicate/noisy results while staying available through the expanded mode, implementation-only provenance remains inspection-only because it has no authored product meaning, and linked legacy variants remain outside ordinary ranking while staying directly addressable. Variant text expansion selects `retrieval_disposition = 'ordinary'`; concrete-key graph/variant/remaster reads may reach direct-only rows. `atlas-record` owns the named graph policy and `atlas-index` lowers it into SQL predicates; neither invents an authorization boundary.
...
Simple product filters lower once through `atlas-domain::SimpleSearchFilter` into the canonical `SearchFilterNode` tree; advanced callers may provide a canonical tree directly. Filters compile to an authoritative SQL keyset whose base relation is `retrieval_disposition = 'ordinary'`, not `RecordVisibility` or its legacy boolean projection, before lexical or vector search. Lookup aliases, variant base-name lookup, dynamic discovery, metric-catalog scope checks, FTS, and vector search compose from that same eligibility rule; direct record-key, remaster-link, and graph reads may still reach direct-only records. SQLite lexical search keeps the keyset in the same query as `records_fts` and the normal search path uses precision FTS lanes over title/alias and high-signal facet columns. `atlas-search` classifies FTS hits by title/alias coverage and high-value record-token coverage before hybrid fusion, so weak broad-token FTS evidence is demoted instead of crowding out stronger semantic matches. The vector table stays rowid plus vector; filtering metadata remains in normal SQLite tables and is reached through `document_embedding_cache.rowid`.
...
Graph context retrieval is one-hop. `atlas graph links <record>` routes through the CLI client into app-service and `AtlasRetrievalService`, resolves a strict name when needed, loads the seed record through the normal record path, asks the `ReferenceReadIndex` boundary for authored non-embedded `reference_edges` across visibility classifications, applies deterministic edge ordering and unique-neighbor limits, then hydrates only retained neighbor records. Copied embedded-description edges require the explicit expanded graph mode so they cannot dominate ordinary context. `atlas graph uses <record>` is the backlinks-focused form. Record-detail app composition uses that same graph owner for one outer `RecordSurfaceView.references` contract. It defaults to outgoing limit 8 and backlink limit 0; a zero direction is tagged `not_requested` without invented totals, while requested directions retain graph ordering, exact totals/truncation, and a typed nonfatal unavailable state. Explicit record-detail backlink limits use the same common owner. Search-compact and encounter composition do not side-load graph context, and internal entity/occurrence membership IDs are not converted into record links. Variant group resolution is owned by `atlas-search`: canonical record keys open the group for that concrete record, including direct-only seeds, while text inputs prefer ordinary-eligible variant base-name group lookup before strict record-name fallback. `atlas similar <record>` is a record-to-record retrieval surface owned by `atlas-search`: runtime opens a vector-ready record retrieval service without loading the embedding model, resolves a seed record, loads the seed's stored parent embedding from the active SQLite index, queries vector candidates without re-rendering or re-embedding the seed, applies the same structured filter scope to candidate retrieval, and reranks/explains the result set with modest shared-reference and shared-trait evidence. The ref-based similar surface resolves strict seed names with that same structured filter so inputs like `atlas similar "Dirge of Doom" --kind spell` choose the spell seed, while the direct key-based similar request remains available for callers that already have a canonical `RecordKey`. Search relationship flags such as `--referenced-by` remain result-set filters; graph context retrieval returns a local context bundle with edge evidence, counts, and truncation metadata.

Runtime SQLite access is read-only and goes through `SqliteIndexReader`, with complete canonical record hydration owned only by `atlas-index::read`. The reader requires the matching body for every schema-v4 NPC, hazard, and spell, rejects cross-family, missing, extra, malformed, or kind-mismatched bodies, hydrates ordered local spell children only on consumables, and verifies family tables plus shared content/reference/exclusion/query projections against their canonical owners. It returns the `atlas-record`-owned `RetrievedRecord { record: AtlasRecord, body: Option<RecordBody>, spell_children: Vec<ConsumableSpellChild> }` aggregate through `RecordReadIndex`. All-record and requested-key paths use the same strict mixed-family contract. Search, app-service, and CLI preserve that aggregate; no consumer side-loads or reconstructs a body from raw JSON, generic mechanics, or query rows.

The reader requires the adjacent manifest before opening the database. Generic metric and actor/item side-table reads explicitly order rows by parent key and persisted ordinal before grouping and apply only to families without a canonical creature, hazard, or standalone-spell body. Creature metrics remain query-only artifact rows, while spell filter facts remain one-way `spell_records`, `spell_traditions`, and `spell_damage_types` projections; requested reads reconcile them against the canonical body instead of attaching either family to `AtlasRecord.mechanics`. Creature activities and spellcasting hydrate only through the canonical body and its entity/occurrence projections. `RecordVisibility` is hydrated only from its typed state/reason columns; the separately persisted retrieval role/disposition/rationale tuple and its `is_default_visible` projection remain routing policy. Pair acquisition takes the target's shared publication lock, rejects a noncanonical manifest digest before path construction, cheaply requires exact envelope, artifact-contract, and schema stamps, and binds a real direct-child generation. New copies are synced, checksummed once, sealed read-only, and atomically installed before immutable SQLite open. Matching sealed local identity records avoid warm whole-file hashes; missing or stale trust is a recovery boundary, owner-writable legacy files are recreated from the retained primary, and links or reparse aliases fail closed. It opens every retained Diesel and validation/hydration connection on that generation before releasing the lock. No validation, readiness, inspection, or hydration operation reopens the visible artifact pathname. Long-lived readers stay on their generation while a publisher installs a new pair, later readers bind to the new generation, and identity-bound lease cleanup cannot delete a replacement path. All-record and keyed hydration validate the rows they request and preserve typed decode failures without launching global canonical coherence. The bound target-validation facade composes one base/global report and any requested vector checks on the same authenticated reader. `Full` validation checks structural and global artifact invariants without decoding every body; broad all-body canonical coherence remains an explicit optional diagnostic and is never an implicit final or CI step. Retrieval orchestration depends on index-owned read capabilities rather than ad hoc SQL; search may request an explicit narrow read trait but must not create a second canonical hydration path. `atlas-index` owns the composite `RetrievalReadIndex` bundle for consumers that legitimately need the full retrieval read surface; `atlas-search` consumes that bundle while exposing product-facing retrieval traits to its own callers.

Construction-time writes are separate and go through `IndexArtifactWriter` implementations such as `SqliteIndexWriter`, which enable SQLite foreign-key enforcement before their transaction and write a temporary artifact only after canonical records/entities/occurrences/relationships/content, ordered record mechanics children, projections, embedding cache rows, and `record_vector_index` are complete. Once all SQLite handles close, the writer syncs and seals the staged file, cheaply checks compatibility metadata, hashes one retained open-file identity once, and returns an opaque single-use publication receipt. Ingest uses that receipt's digest for the adjacent manifest and passes the live receipt into the index-owned pair publisher. The publisher uses a bounded exclusive-lock acquisition, verifies recovery candidates only at the recovery boundary, creates, seals, atomically installs, and hashes one byte-copy generation only when absent, reuses matching trusted local generations without whole-file hashing, snapshots the already verified prior pair without a second hash, and restores or recovers that pair after publication failure or interruption. Product surfaces route retrieval and filter discovery through the CLI client/app-service boundary, `atlas-runtime`, and `AtlasRetrievalService`; they do not open SQLite or assemble retrieval dependencies directly.
