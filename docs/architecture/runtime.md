...
Spell record detail may carry one selected-form result requested by an opaque backend-provided form ID paired with a cast rank. App-service validates the pair, resolves its server-owned overlay context, and calls the canonical spell resolver rather than exposing overlay locators or reconstructing patches in the client.
