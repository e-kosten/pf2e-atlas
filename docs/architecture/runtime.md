...
Authoritative source coverage is an ingest-owned, test-evidence contract under `contracts/source-leaf-coverage/v1`. A declaration matches only an exact source contract, document class, discriminator, role, parent context, and normalized leaf. Recursive or prefix declarations are forbidden; `[]` preserves array-member order and multiplicity, and `*` is valid only for a true map with an explicit key policy. Focused tests emit actual-read receipts using the source DTO `SourcePresence` states and compare type, value, member identity, order, and multiplicity through each declared final owner. Declarations, diagnostics, transient metrics, and raw clones cannot stand in for those receipts.
