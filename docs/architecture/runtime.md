...
Graph context retrieval is key-based and one-hop in the V1 Rust CLI. `atlas graph links <record>` routes through `AtlasRetrievalService`, resolves names above the index backend when needed, loads the seed record through the normal record path, asks the graph-product index trait for policy-visible `reference_edges`, applies deterministic edge ordering and unique-neighbor limits, then hydrates only retained neighbor records. Search relationship flags such as `--referenced-by` remain result-set filters; graph context retrieval returns a local context bundle with edge evidence, counts, and truncation metadata. Relational-friendly graph product commands such as `atlas graph variants <record-or-base-name>` and `atlas graph remaster <record>` use the same graph-product index trait rather than the ranked search trait. Variant lookup first uses normal record-reference resolution and then, only for the variants command, falls back to exact `variant_base_name` group resolution so base names such as `Dread Ampoule` can return their group without becoming global record aliases.
