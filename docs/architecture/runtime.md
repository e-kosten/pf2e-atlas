...
Graph context retrieval is key-based and one-hop in the V1 Rust CLI. `atlas graph links <record>` routes through `AtlasRetrievalService`, resolves names above the index backend when needed, loads the seed record through the normal record path, asks the graph-product index trait for policy-visible `reference_edges`, applies deterministic edge ordering and unique-neighbor limits, then hydrates only retained neighbor records. `atlas graph uses <record>` is the backlinks-only product surface over the same graph context path. `atlas graph expand <query>` is the semantic-to-graph expansion surface: it runs semantic retrieval, infers referenced mechanics from those semantic seeds, keeps mechanics that meet the requested support threshold, and expands back to records using those mechanics with evidence. Graph expansion accepts the same `SearchFilterNode` scope as search; the filter is applied to semantic seed retrieval and to the final expanded candidate keyset through the index filter compiler rather than duplicated command-specific filtering logic. `atlas similar <record>` is a record-to-record retrieval surface owned by `atlas-search`: it resolves a seed record, loads that record's stored parent embedding from the active index backend, queries vector candidates without re-rendering or re-embedding the seed, applies the same structured filter scope to candidates, and reranks/explains the result set with shared mechanics and trait overlap. Search relationship flags such as `--referenced-by` remain result-set filters; graph context retrieval returns a local context bundle with edge evidence, counts, and truncation metadata. Relational-friendly graph product commands such as `atlas graph variants <record-or-base-name>` and `atlas graph remaster <record>` use the same graph-product index trait rather than the ranked search trait. Variant lookup first uses normal record-reference resolution and then, only for the variants command, falls back to exact `variant_base_name` group resolution so base names such as `Dread Ampoule` can return their group without becoming global record aliases.
