...
`atlas-record::CreatureRecord` is the durable storage-neutral creature-family subtype. Its B3 core owns typed source-presence and provenance for identity/classification, source adjustment and alliance, legacy ability facts, perception and source initiative selection, languages, standard and Lore skills, defenses and IWR, movement, and resources. Source alliance is an intrinsic fact only; it does not set encounter side or any other runtime behavior. Closed vocabularies use enums; provider-defined slugs use validated newtypes; ambiguous legacy shapes remain explicit unsupported values with conversion diagnostics. Stable source-backed component IDs and authored collection order are stored separately. When a valid open source value cannot be used directly as a component ID, ingest preserves the value and derives a reversible record-scoped source fallback; absent or null component identity uses the existing diagnosed owner/family/ordinal fallback. Neither case discards the enclosing record. Diagnostics use stable codes plus relative record/source/field identity, observed value, disposition, and owner rather than absolute paths or formatted errors. Serialized shield HP and resource values are retained as provenance-only inputs because mutable current values belong to runtime instances. The legacy resource `maxx` drift is preserved as a typed unsupported source fact with a portable diagnostic and is never coerced into `max`. B4 extends this same subtype with embedded entities and occurrences; it does not create a parallel creature model.

Source normalization emits ingest construction facts beside each normalized record. These facts retain the complete versioned NPC Source envelope and carry the canonical creature body for B4 and the later atomic C1 writer, source identity such as slugs and compendium-source locators, embedded item identity/provenance/content references, and journal page content parsed from Foundry source JSON. The typed envelope preserves valid sibling fields and the immutable provenance/audit payload without becoming a raw semantic fallback. The current pre-C1 sparse actor-row values are a one-way projection from `CreatureRecord`, not a second parser, canonical store, or hydration fallback. Later ingest phases use construction facts for aliases, remaster links, and source-backed generated records instead of reparsing `AtlasRecord.raw_json`; reference, FTS, and embedding projections consume the normalized `RichDocument` outputs produced during normalization. Persisted raw JSON remains provenance/debug input and a future analysis substrate, not the normal construction API between ingest phases.
...
Pinned-base generated-affliction construction overloads `is_default_visible`/`visible_by_default` to distinguish a generated canonical from its source instance in `source_pipeline.rs` and `generated/afflictions/{mod.rs,records.rs}`. The Checkpoint B target replaces that coupling with an explicit canonical/source-instance role and typed host-instance-canonical relationships. Canonicals own deduplicated user-facing meaning and ordinary ranking; source instances preserve exact host occurrence/provenance and remain directly addressable and graph-reachable while staying outside ordinary ranking solely to prevent duplicate canonical results. B4 owns construction, C1 owns persistence/validation, D1 owns retrieval disposition, and D3 audits the one-canonical/one-instance/three-relationship fixture before UI work.
...
Runtime SQLite access is read-only and goes through `SqliteIndexReader`, with complete canonical record hydration owned only by `atlas-index::read`. Retrieval orchestration depends on index-owned read capabilities rather than ad hoc SQL; search may request an explicit narrow read trait but must not create a second canonical hydration path. `atlas-index` owns the composite `RetrievalReadIndex` bundle for consumers that legitimately need the full retrieval read surface; `atlas-search` consumes that bundle while exposing product-facing retrieval traits to its own callers. Construction-time writes are separate and go through `IndexArtifactWriter` implementations such as `SqliteIndexWriter`, which write a temporary artifact and publish it only after canonical records/entities/occurrences/content, projections, embedding cache rows, and `record_vector_index` are complete. Product surfaces route retrieval and filter discovery through the CLI client/app-service boundary, `atlas-runtime`, and `AtlasRetrievalService`; they do not open SQLite or assemble retrieval dependencies directly.
