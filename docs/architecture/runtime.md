...
Authoritative source coverage is an ingest-owned, test-evidence contract under `contracts/source-leaf-coverage/v1`. A declaration is bound to the pinned source contract/commit/signature and one exact tuple from the authenticated 313-entry `contracts/pf2e-type-registry.yaml`; it then owns one normalized leaf. Recursive or prefix declarations are forbidden; `[]` uses independent ordered member receipts, and `*` is valid only for a true map with an enforced key policy. Focused tests authenticate fixture record/path/digests and emit receipts by running mutation-sensitive source and final-owner accessors. Receipt sets reconcile source prevalence and compare the shared DTO `SourcePresence` state, type, value, member identity, order, and multiplicity through each declared final owner. Declarations, diagnostics, transient metrics, raw clones, and caller-supplied success booleans cannot stand in for those receipts.
