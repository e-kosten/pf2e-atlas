...
`atlas-record::project_creature_mechanics` exposes the same canonical creature body as storage-neutral mechanics base facts. It covers defenses, awareness and skills, movement, resource maxima, strikes, actions, spellcasting entries, spells, rolls and DCs, damage, action costs, frequencies, uses, spell slots, and actor ritual DC. Targets are composed from typed component and occurrence identities; labels, metric-key prefixes, opaque-ID parsing, and prose never determine canonical target identity. Structured target identities use one versioned, hex-component encoding whose closed variant tag and escaped UTF-8 components make the public string identity injective without a compatibility identifier. Unsupported source shapes, capability notes, and unsupported-only Lore or Equipment occurrence notes remain explicit structured projection values associated with their target or occurrence. This projection contains source/base facts only: mutable current values, turn state, and action budgets remain outside the canonical artifact and belong to app-service runtime policy. The production FTS row and embedding parent presentation for a canonical creature consume this projection directly, together with the canonical actor-side categories from `project_creature_facts`; legacy `AtlasRecord.mechanics` remains a compatibility/storage projection and is not an independent search-mechanics source for those records.
