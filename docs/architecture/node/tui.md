...
- editorial workbench entrypoints for tag-review tasks, including the row-centric ontology translation review session/import workflow
...
Search-hosted filter explorer value ordering is presentation state. Backend discovery services may return count-bearing value domains in their natural discovery order, while `src/tui/search-screen/filter-explorer-value-sorting.ts` owns per-session value-list sorting for the TUI. The default order is semantic: canonical field order for rarity and action cost, numeric order for numeric values, true-first for booleans, and alphabetical display-label order for ordinary text, enum, and set values. Frequency ordering is an action-rail toggle for eligible value levels only; it is not a backend query mode and it does not sort field groups, derived-tag hierarchies, metric namespaces, or record children. Reconciliation of live refreshed counts uses the same sorter so selected zero-count values stay visible without jumping into a separate selected bucket.
...
- `src/tui/search-screen/structured-draft/structured-draft-edit-routes.ts` for classifying semantic field-edit intents into grouped-field or leaf routes
- `src/tui/search-screen/structured-draft/structured-draft-edit-route-actions.ts` for executing those routes against prompt, explorer, and host-mutation owners
...
Structured field editing has two semantic edit routes. Group-field routes edit the same-field cohort in the containing canonical boolean group and always write through grouped-field replacement. This route covers metadata set and discrete shared-explorer fields, plus pack, rarity, and action cost. Leaf routes edit one semantic clause through node replacement, append, or a leaf-specific singleton placement rule. Scope is a root-singleton leaf: the route places one root scope clause, while query-state normalization prunes scope-dependent metadata, metric, and action-cost clauses when the category changes. Metrics are ordinary leaf routes; shared-explorer metric discovery is a child surface, not an exact-node fallback route. Canonical record-link leaves, `linksTo` and `linkedFrom`, are executable leaf routes that prompt for a record key and replace the selected link clause.

The route owner accepts semantic route intents, not TUI picker rows. Field intents name a durable field reference such as metadata field, metric family, or pack plus the target `groupPath`; leaf intents name the leaf kind plus the target `groupPath`. The route owner resolves those intents through one route catalog backed by shared metadata semantics and scoped discovery availability. `Pf2eTerminalQueryFieldOption` remains a display/session model for pickers and explorer setup, but it is not the router contract and callers do not choose route behavior from `fieldType`, `editor`, or display labels. Prompt and entry-action owners pass field references or leaf intents into the route owner before they open prompts, explorers, or write back mutations.

Structured-editor writeback flows emit bounded mutation families such as replace node, append nodes, and replace grouped field before final writeback. Shared-explorer writeback must provide a host mutation builder; it cannot fall back to generic explorer serialization. Prompt-local and select-target child surfaces may still collect explorer field state for scalar or metric-key prompts, but those named child-surface paths do not perform structured-editor writeback. Query-global replacement belongs only to explicitly query-global search flows. Generic helpers in `src/tui/filter-explorer/search-draft-query.ts` and `src/tui/search/service.ts` may keep preparing explorer drafts and generic insertion results, but they do not own structured-editor mutation or resume semantics. The lower search-hosted explorer API reports one generic session event stream to the structured-editor continuation owner instead of exposing separate search-editor continuation callbacks for back, cancel, exit, live change, and select-target handling.
...
This seam is enforced by named owner tests for the continuation coordinator, route classifier, route executor, resume-target state, grouped-field helpers, and broad search-screen interaction flows. Lint also blocks structured-draft code from calling the generic `applyFilterExplorerDraft` writeback helper directly, opening explorer continuation outside its owner, using retired pack/rarity prompt wrappers, reintroducing the retired exact-node shared-explorer fallback, reintroducing generic structured-draft serializer fallback symbols, or adding pack/rarity/action-cost route branches and node construction in action owners.
...
During live query edits, the filter explorer keeps the current model mounted and treats displayed counts as the snapshot for the active editing session. Query edits update the host query immediately, clear cached count models, and schedule a debounced same-mode refresh with stale-result protection. Prepared discovery preloads only the single active field needed for direct-value picker rendering; multi-field prepared surfaces return lazy child sources and do not count fields the user has not opened. When an edit changes one discrete field inside a multi-field surface, the refresh target is narrowed to that field so refreshed counts can merge back without forcing unrelated fields. The visible picker remains interactive with old counts while refresh work runs; refreshed counts merge back without allowing older completions to replace newer query state. Discovery-mode changes and reopened explorer sessions may also load fresh matching or catalog counts. This keeps picker input responsive while avoiding broad synchronous count recomputation on the Ink event loop for each edit.
