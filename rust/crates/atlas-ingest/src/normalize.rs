...
use atlas_domain::{
    PackName, PublicationFamily, RecordFamily, RecordId, RecordKey, TextStatus, TimeKind, TimeUnit,
};
use serde_json::Value;

use crate::references::extract_reference_candidates;
use crate::{IngestError, LoadedRecord, ManifestPack, NormalizedTime, metrics};
...
pub(crate) fn time_with_kind(kind: TimeKind, text: &str) -> NormalizedTime {
    NormalizedTime {
        kind,
        actions: None,
        duration_value: None,
...
pub(crate) fn parse_duration_unit(value: &str) -> Option<(i64, TimeUnit)> {
    let mut parts = value.split_whitespace();
...
pub(crate) fn canonical_time_unit(value: &str) -> Option<TimeUnit> {
