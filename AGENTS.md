...
- Create agent worktrees in this repository-local gitignored directory first: `<repo-root>/.worktrees/<task-slug>` (under the current repo root). This should be your first choice because it is portable across environments.
- If the repository-local worktree directory is unavailable in your session, create worktrees under a writable sandbox root named in the current environment instructions. Use that literal path, not a symlink-resolved equivalent (for example, `/tmp` instead of `/private/tmp`).
- Prefer a deterministic worktree path directly under that allowed root, for example `.worktrees/pathfinder-mcp-worktree-<task>` or `<writable-root>/pathfinder-mcp-worktree-<task>`. Do not place agent worktrees anywhere else unless the instructions for the current environment explicitly name another writable root.
...
- Commit and validate the change inside the worktree only when the user asks to prepare a commit. For large tasks, milestone commits may accumulate in the worktree after explicit prepare-commit passes, but the worktree is not merge-ready until the full planned workset is complete.
