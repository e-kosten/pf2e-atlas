...
Independent validation should be skeptical and grounded in the actual diff, architecture invariants, and production call paths. Reviewers should batch related findings, use adversarial or mutation checks where they can expose plausible failures, separate pre-existing baseline failures from candidate regressions, and never silently waive either. Source/artifact identity and candidate provenance remain required at the trust boundaries where those identities are consumed; ordinary task coordination does not require authorization shell scripts, per-amendment checksum packages, or a new launcher/framework.
