...
describe("Pf2eDataService / Search and Lookup", () => {
  ...
  it("supports lookup, listing, filtering, and derived metadata", async () => {
    ...
    expect(service.listRecords({ category: "creature", metadata: { field: "derivedTags", op: "includesAny", values: ["celestial_setting"] } }).records.map((record) => record.name)).toEqual(expect.arrayContaining(["Empyreal Dragon", "Rekhep", "Guloval", "Ghaele", "Monadic Deva"]));
    ...
    expect(service.listRecords({ category: "creature", metadata: { field: "derivedTags", op: "includesAny", values: ["fiendish_setting"] } }).records.map((record) => record.name)).toEqual(expect.arrayContaining(["Insidiator", "Vrock", "Cacodaemon", "Agradaemon", "Ferrugon", "Babau", "Cythnigot"]));
    ...
    expect(monadicDeva?.derivedTags).toContain("celestial_setting");
    expect(monadicDeva?.derivedTags).not.toContain("heaven_setting");
    const kadamel = service.lookup("Kadamel", { category: "creature" }).match;
    expect(kadamel?.derivedTags).toEqual(expect.arrayContaining(["heaven_setting", "celestial_setting"]));
    const draconal = service.lookup("Draconal", { category: "creature" }).match;
    expect(draconal?.derivedTags).toEqual(expect.arrayContaining(["nirvana_setting", "celestial_setting"]));
    const bralani = service.lookup("Bralani", { category: "creature" }).match;
    expect(bralani?.derivedTags).toEqual(expect.arrayContaining(["elysium_setting", "celestial_setting"]));
