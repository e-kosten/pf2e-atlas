...
describe("Pf2eDataService / Search and Lookup", () => {
  ...
  it("supports lookup, listing, filtering, and derived metadata", async () => {
    ...
    expect(service.listRecords({ category: "creature", metadata: { field: "derivedTags", op: "includesAny", values: ["coastal_setting"] } }).records.map((record) => record.name)).toEqual(expect.arrayContaining(["Coastal Prowler", "Ship Captain"]));
    ...
    expect(gathganara?.derivedTags).toEqual(expect.arrayContaining(["freshwater_setting", "aquatic_setting", "fey_threat"]));
    const defacedNaiadQueen = service.lookup("Defaced Naiad Queen", { category: "creature" }).match;
    expect(defacedNaiadQueen?.derivedTags).toEqual(expect.arrayContaining(["freshwater_setting", "aquatic_setting", "fey_threat"]));
    ...
    expect(cairnWight?.derivedTags).toEqual(expect.arrayContaining(["graveyard_setting", "undead_threat"]));
