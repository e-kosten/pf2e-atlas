...
describe("Pf2eDataService / Load and Index", () => {
  ...
  it("loads an unchanged SQLite index and requires explicit rebuild when the source changes", async () => {
    ...
    expect(firstService.getStats()).toEqual({ packCount: 16, recordCount: 248 });
    firstService.close();
    ...
    expect(unchangedService.getStats()).toEqual({ packCount: 16, recordCount: 248 });
    ...
    expect(rebuiltService.getStats()).toEqual({ packCount: 16, recordCount: 249 });
