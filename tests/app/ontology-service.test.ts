...
function createDataService(): Pick<Pf2eDataService, "getSearchVocabulary" | "listFilterValues"> {
  return {
    ...
    listFilterValues: ({ field, category }) => {
...
describe("application ontology service", () => {
  ...
  it("builds valid field-specific browse queries for search semantics values", () => {
    ...
    expect(findNodeById(domain.rootNodes, "spell:saveType:fortitude")?.query?.filters.metadata).toEqual({
      field: "saveType",
      op: "eq",
      value: "fortitude",
    });
    expect(findNodeById(domain.rootNodes, "spell:sustained:true")?.query?.filters.metadata).toEqual({
      field: "sustained",
      op: "eq",
      value: true,
    });
    expect(findNodeById(domain.rootNodes, "equipment:hands:1")?.query?.filters.metadata).toEqual({
      field: "hands",
      op: "eq",
      value: 1,
    });
